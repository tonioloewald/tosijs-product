...
- **Demo media lives on the CDN, not in this repo.** Reference `https://cdn.tosijs.net/tosijs-product/<file>` (namespace source: `../static-assets/assets/tosijs-product/`; publish with `bun run deploy` there). They used to sit in `demo/assets` **and** in the committed `docs/` that Pages serves, so every byte was carried twice. `staticDirs: ["demo/assets"]` still flattens what remains (just `favicon.svg`) to `/`.
...
- **`--doc-content-overflow: visible`** is how a page hosting the engine escapes the inline `overflow: hidden` that would kill `position: sticky` (tosijs-ui#119, shipped 1.14.1 — no more `!important`). On a **`full-screen`** page the layout sets that variable to `auto` at (0,2,1) and `auto` collapses the runway, so match `tosi-doc-system[data-layout="full-screen"] .doc-content:has(…)` to outrank it; `full-width` pages need only `.doc-content:has(…)`.
...
- **`docs/version.json` converges — a rebuild no longer dirties the tree.** It used to stamp
  `git rev-parse --short HEAD` unconditionally, so the stamp was always one commit behind and no
  build→commit cycle could settle. [tosijs-ui#122](https://github.com/tonioloewald/tosijs-ui/issues/122)
  fixed that in **1.14.1**: the build hashes the rest of the output into a `contentHash` field and
  **preserves the previous stamp verbatim when nothing else changed**, so `commit` now names the
  commit at which the site content last actually changed, not the parent of the newest one. A stamp
...
- `src/tosi-b3d-scroll.ts` — `TosiScrollCamera`, `TosiScrollTime`, `TosiScrollAnimation` (scroll controllers; use `<tosi-waypoint>` children for camera keyframes). **Scene-agnostic by duck-typing:** `findScene()` walks for any element with a `.scene` property and the controllers touch only `scene.activeCamera` / `scene.animationGroups`, so they work in **tosijs-3d's `<tosi-b3d>`** unchanged. Target tosijs-3d — tosijs-ui's `<tosi-3d>` is the legacy path and its 3D is expected to be deprecated. `TosiScrollTime` was **always** tosijs-3d-only (it drives a `<tosi-b3d-skybox>`); the docs said tosijs-ui until 0.8.0.
