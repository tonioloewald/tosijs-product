(() => {
  ...
  function n(E) {
    ...
    else if (Array.isArray(E)) return E.map(n);
    let M = {};
    for (let L in E) {
      let f = E[L];
      if (E != null && typeof E === "object") M[L] = n(f);
      else M[L] = f;
    }
    return M;
  }
  ...
  function SM(E, M) {
    let L = cE.get(E);
    if (L === undefined) (L = new Set()), cE.set(E, L);
    L.add(M);
  }
  function NM(E) {
    return cE.get(E);
  ...
  var kM = new Set();
  function R(E, M) {
    if (!kM.has(E)) console.warn(M), kM.add(E);
  }
  function gE(E, M) {
    let L = false;
    return (...f) => {
      if (!L) console.warn(M), (L = true);
      return E(...f);
    };
  }
  var j = (E) => {
  ...
  var LE = (E) => {
    ...
    if (M instanceof Element) {
      let L = b.get(E),
        f = EE.get(E);
      if (L != null) b.set(M, n(L));
      if (f != null) EE.set(M, n(f));
    }
    for (let L of Array.from(
      E instanceof HTMLTemplateElement ? E.content.childNodes : E.childNodes
    ))
      if (L instanceof Element || L instanceof DocumentFragment)
        M.appendChild(LE(L));
      else M.appendChild(L.cloneNode());
    return M;
  };
    ...
    if (Array.isArray(E)) return E;
      ...
      while (E.length > 0) {
        ...
        if (L === -1) {
          ...
          let f = E.slice(0, L);
          if (((E = E.slice(L)), f !== "")) M.push(f.split("."));
          if (
            ((L = E.indexOf("]") + 1),
            M.push(E.slice(1, L - 1)),
            E.slice(L, L + 1) === ".")
          )
            L += 1;
          E = E.slice(L);
  ...
  var h = new WeakMap();
  function bM(E, M) {
    if (h.get(E) === undefined) h.set(E, {});
    if (h.get(E)[M] === undefined) h.get(E)[M] = {};
    let L = h.get(E)[M];
    if (M === "_auto_")
      E.forEach((f, H) => {
        if (f[nE] === undefined) f[nE] = iE();
        L[f[nE] + ""] = H;
      });
    else
      E.forEach((f, H) => {
  ...
  function NL(E, M) {
    if (h.get(E) === undefined || h.get(E)[M] === undefined) return bM(E, M);
    else return h.get(E)[M];
  }
  function xL(E, M, L) {
    L = L + "";
    let f = NL(E, M)[L];
    if (f === undefined || V(E[f], M) + "" !== L) f = bM(E, M)[L];
    return f;
  }
  function yL(E, M, L) {
    if (E[M] === undefined && L !== undefined) E[M] = L;
    return E[M];
  }
  function cM(E, M, L, f) {
    let H = M !== "" ? xL(E, M, L) : L;
    if (f === hE) return E.splice(H, 1), h.delete(E), Symbol("deleted");
    else if (f === PM) {
      if (M === "" && E[H] === undefined) E[H] = {};
    } else if (f !== undefined)
      if (H !== undefined) E[H] = f;
      else if (M !== "" && V(f, M) + "" === L + "")
        E.push(f), (H = E.length - 1);
      else throw Error(`byIdPath insert failed at [${M}=${L}]`);
    return E[H];
  }
  function xM(E) {
    if (!Array.isArray(E))
  ...
  function V(E, M) {
    ...
    for (H = 0, $ = L.length; f !== undefined && H < $; H++) {
      ...
      if (Array.isArray(Z))
        ...
        if (((f = f[Number(Z.slice(1))]), Z[0] !== "=")) return;
      } else if (Z.includes("=")) {
        let [Y, ...F] = Z.split("=");
        f = cM(f, Y, F.join("="));
      } else (J = parseInt(Z, 10)), (f = f[J]);
    }
    return f;
  }
  function l(E, M, L) {
      ...
      if (typeof $ === "string") {
        ...
        while ($.length > 0) {
          ...
          if ($.length > 0 || H.length > 0)
            ...
            if (L !== hE) {
              if (f[J] === L) return false;
              f[J] = L;
            } else {
              if (!Object.prototype.hasOwnProperty.call(f, J)) return false;
              delete f[J];
            }
            return true;
          }
        }
      } else throw Error(`setByPath failed, bad path ${M}`);
    }
  ...
  var GE = () => {
    if (sE === null) throw Error("xin proxy not initialized");
    return sE;
  };
  var pE = null;
  var rE = null;
  ...
  var gM = () => {
    if (pE === null) throw Error("bind not initialized");
    return pE;
  };
  var dM = () => {
    if (rE === null) throw Error("on not initialized");
    return rE;
  };
  ...
  class hM {
    ...
    callback;
    constructor(E, M) {
      let L = typeof M === "string" ? `"${M}"` : `function ${M.name}`,
        f;
      if (typeof E === "string")
        (this.test = (H) =>
          typeof H === "string" &&
          H !== "" &&
          (E.startsWith(H) || H.startsWith(E))),
          (f = `test = "${E}"`);
  ...
  var mL = () => {
    if (fE.perf) console.time("xin async update");
    let E = Array.from(YE);
    ...
    for (let M of E)
      VE.filter((L) => {
        ...
        try {
          ...
          console.error(
            `Listener ${L.description} threw "${H}" handling "${M}"`
          );
        }
        if (f === nM) HE(L);
      });
    if (typeof tE === "function") tE();
    if (fE.perf) console.timeEnd("xin async update");
  };
  var N = (E) => {
    let M = typeof E === "string" ? E : j(E);
    if (M === undefined)
      throw (
        (console.error("touch was called on an invalid target", E),
    ...
    if (oE === false)
      (lE = new Promise((f) => {
        ...
        (oE = setTimeout(mL));
    if (YE.find((f) => M.startsWith(f)) == null) YE.push(M);
    let L = M.match(/^(.+)\[(\d+)\](.*)$/);
    if (L !== null) {
      let [, f, H, $] = L,
        J = parseInt(H, 10),
      ...
      if (Q != null) {
        let Z = PL(f, J, Q, $);
        for (let Y of Z)
          if (YE.find((F) => Y.startsWith(F)) == null) YE.push(Y);
      }
    }
  };
  var i = (E, M) => {
  ...
  var sM = (E) => {
    if (E instanceof HTMLInputElement) return E.type;
  ...
  var eE = (E, M, L = true) => {
    if (E != null && M != null)
      if (typeof M === "string") E.textContent = M;
      else if (Array.isArray(M))
        M.forEach((f) => {
          E.append(f instanceof Node && L ? LE(f) : f);
        });
      else if (M instanceof Node) E.append(L ? LE(M) : M);
      else throw Error("expect text content or document node");
  };
  var UE = (E, M = 250) => {
    let L;
    return (...f) => {
      if (L !== undefined) clearTimeout(L);
      L = setTimeout(() => {
        E(...f);
      }, M);
    };
  ...
  var vL = {
    RADIANS_TO_DEGREES: bL,
    DEGREES_TO_RADIANS: cL,
    clamp: c,
    lerp: s,
  };
  function MM(E, M = document.body) {
    let L = getComputedStyle(M);
    if (E.endsWith(")") && E.startsWith("var(")) E = E.slice(4, -1);
        ...
        $ =
          H !== 0
            ? f === E
              ...
              : 4 + (E - M) / H
            : 0;
      (this.h = 60 * $ < 0 ? 60 * $ + 360 : 60 * $),
        (this.s =
          H !== 0 ? (f <= 0.5 ? H / (2 * f - H) : H / (2 - (2 * f - H))) : 0),
        (this.l = (2 * f - H) / 2);
    }
  }
  var t =
  ...
  class X {
    ...
    static fromCss(E) {
      ...
      if (M) {
        let Z = M[1],
          Y = (F, z) => parseInt(Z.slice(F, F + z), 16);
      ...
      if (t instanceof HTMLSpanElement)
        (t.style.color = "black"),
          ...
          (L = getComputedStyle(t).color),
          t.remove();
      let [f, H, $, J] = L.match(/[\d.]+/g) || ["0", "0", "0", "0"],
        Q = L.startsWith("color(srgb") ? 255 : 1;
      return new X(
        Number(f) * Q,
        Number(H) * Q,
        Number($) * Q,
    ...
    static fromHsl(E, M, L, f = 1) {
      ...
      if (M === 0) H = $ = J = L;
        ...
        let Z = (G, B, W) => {
            if (W < 0) W += 1;
            if (W > 1) W -= 1;
            if (W < 0.16666666666666666) return G + (B - G) * 6 * W;
            if (W < 0.5) return B;
            if (W < 0.6666666666666666)
              return G + (B - G) * (0.6666666666666666 - W) * 6;
            return G;
          },
          Y = L < 0.5 ? L * (1 + M) : L + M - L * M,
          F = 2 * L - Y,
          z = (((E % 360) + 360) % 360) / 360;
        (H = Z(F, Y, z + 0.3333333333333333)),
          ($ = Z(F, Y, z)),
          (J = Z(F, Y, z - 0.3333333333333333));
      }
    ...
    get rgba() {
      ...
      return `rgba(${E.toFixed(0)},${M.toFixed(0)},${L.toFixed(0)},${f.toFixed(
        2
      )})`;
    }
    get RGBA() {
    ...
    get hsl() {
      let { h: E, s: M, l: L } = this._hsl;
      return `hsl(${E.toFixed(0)}deg ${(M * 100).toFixed(0)}% ${(
        L * 100
      ).toFixed(0)}%)`;
    }
    get hsla() {
      ...
      return `hsl(${E.toFixed(0)}deg ${(M * 100).toFixed(0)}% ${(
        L * 100
      ).toFixed(0)}% / ${(this.a * 100).toFixed(0)}%)`;
    }
    get mono() {
    ...
    toString() {
      return this.a === 1
        ? "#" + JE(this.r) + JE(this.g) + JE(this.b)
        : "#" +
            JE(this.r) +
            JE(this.g) +
            JE(this.b) +
            JE(Math.floor(255 * this.a));
    }
    brighten(E) {
    ...
    static blendHue(E, M, L) {
      let f = (M - E + 720) % 360;
      if (f < 180) return E + L * f;
      else return E - (360 - f) * L;
    }
    mix(E, M) {
      let L = this._hsl,
        f = E._hsl;
      return X.fromHsl(
        L.s === 0 ? f.h : f.s === 0 ? L.h : X.blendHue(L.h, f.h, M),
        s(L.s, f.s, M),
        s(L.l, f.l, M),
        s(this.a, E.a, M)
      );
    ...
    colorMix(E, M) {
      return X.fromCss(
        `color-mix(in hsl, ${this.html}, ${E.html} ${(M * 100).toFixed(0)}%)`
      );
    }
    ...
    static registerComputedColor(E, M, L, f) {
      if (!X.computedColors.has(E))
        X.computedColors.set(E, { varName: M, scale: L, method: f }),
          X.queueRecompute();
    }
    static queueRecompute() {
      if (X.recomputeQueued) return;
      (X.recomputeQueued = true),
        queueMicrotask(() => {
          (X.recomputeQueued = false), X.recomputeColors();
        });
    }
    static recomputeColors() {
      if (X.computedColors.size === 0) return;
      let E = [];
...
`)}
      ...
      if (X.computedColorStylesheet === null)
        (X.computedColorStylesheet = document.createElement("style")),
          (X.computedColorStylesheet.id = "tosijs-computed-colors"),
          document.head.append(X.computedColorStylesheet);
  ...
  var lM = false;
  function gL() {
    if (!lM) (lM = true), RE.add(() => X.queueRecompute());
  }
  function dL(E) {
  ...
  var LM = (E, M) => {
    if (typeof M === "number" && !iL.test(E)) M = `${M}px`;
    if (E.startsWith("_"))
      if (E.startsWith("__"))
...
${H}
${L}  }`;
    } else return sL(L, f, M);
  };
  var $E = (E, M = "") => {
    return Object.keys(E).map((f) => {
      let H = E[f];
      if (typeof H === "string") {
        if (f === "@import") return `@import url('${H}');`;
...
`);
  ...
  var rL = (E) => {
    ...
    for (let L of Object.keys(E)) {
      let f = E[L];
      if (f instanceof X) M[L] = f.inverseLuminance;
      else if (
        typeof f === "string" &&
        f.match(/^(#[0-9a-fA-F]{3}|rgba?\(|hsla?\()/)
      )
  ...
  var fM = new Proxy(
    ...
    {
      get(E, M) {
        if (M === "default") return kE;
        if (E[M] == null) {
          M = P(M);
          let [, L, , f, H, $] = M.match(/^([-\w]*?)((_)?(\d+)(\w?))?$/) || [
              "",
              M,
            ],
            J = `--${L}`;
          if (H != null) {
            ...
            switch ($) {
              ...
              default:
                throw (
                  (console.error($),
                  Error(`Unrecognized method ${$} for css variable ${J}`))
                );
            }
          } else E[M] = `var(${J})`;
        }
  ...
  function aM() {
    ...
    return {
      colorScheme: E("(prefers-color-scheme: dark)") ? "dark" : "light",
      contrast: E("(prefers-contrast: more)")
        ? "more"
        : E("(prefers-contrast: less)")
        ? "less"
        : E("(prefers-contrast: custom)")
        ? "custom"
        : "no-preference",
      reducedMotion: E("(prefers-reduced-motion: reduce)"),
      reducedTransparency: E("(prefers-reduced-transparency: reduce)"),
      forcedColors: E("(forced-colors: active)"),
    };
  }
  function oL(E) {
    ...
    let M = [
      ...
      L = () => E(aM()),
      f = M.map((H) => matchMedia(H));
    for (let H of f) H.addEventListener("change", L);
    return () => {
      for (let H of f) H.removeEventListener("change", L);
    };
  }
  ...
  var fL = (E, M, L) => {
    let f = LM(P(M), L);
    if (f.prop.startsWith("--")) E.style.setProperty(f.prop, f.value);
    else E.style[M] = f.value;
  };
  var lL = (E) => {
    return {
      toDOM(M, L) {
    ...
    if (M === "style")
      if (typeof L === "object")
        for (let f of Object.keys(L))
          if (j(L[f])) v(E, L[f], lL(f));
          else fL(E, f, L[f]);
      else E.setAttribute("style", L);
    else {
      let f = P(M),
        H = E.constructor.observedAttributes;
      if (H?.includes(M) || H?.includes(f))
        if (typeof L === "boolean")
          L ? E.setAttribute(f, "") : E.removeAttribute(f);
        else E.setAttribute(f, L);
  ...
  var BE = (E, M, L) => {
    if (M === "apply") L(E);
      ...
      if (
        (typeof L.binding === "string" ? TE[L.binding] : L.binding) !==
          undefined &&
        L.value !== undefined
      )
        v(
          E,
          L.value,
          L.binding instanceof Function ? { toDOM: L.binding } : L.binding
        );
      else throw Error("bad binding");
      ...
      if (f !== "value") {
        ...
        if ($)
          R(
            `bind${f}`,
            `bind${M.substring(4)} is deprecated. Use { ${$}: ... } instead.`
          );
      }
      let H = TE[f];
      if (H !== undefined) v(E, L, H);
      else throw Error(`${M} is not allowed, bindings.${f} is not defined`);
    } else if (L != null && typeof L === "object" && L[ME]) v(E, L, ML(M));
    else if (j(L)) v(E, L, ML(M));
    else HL(E, M, L);
  };
  var JM = (E, ...M) => {
    if (IE[E] === undefined) {
      let [H, $] = E.split("|");
      if ($ === undefined) IE[E] = globalThis.document.createElement(H);
      else IE[E] = globalThis.document.createElementNS($, H);
    }
    let L = IE[E].cloneNode(),
      f = {};
    for (let H of M)
      if (
        ...
        if (L instanceof HTMLTemplateElement) L.content.append(H);
        else L.append(H);
      else if (j(H)) L.append(I.span({ bindText: H }));
  ...
  var I = new Proxy(
    { fragment: $M },
    {
      get(E, M) {
        if (
          ((M = M.replace(/[A-Z]/g, (L) => `-${L.toLocaleLowerCase()}`)),
          E[M] === undefined)
        )
          E[M] = (...L) => JM(M, ...L);
        return E[M];
  ...
  var ZM = new Proxy(
    ...
    {
      get(E, M) {
        if (E[M] === undefined) E[M] = (...L) => JM(`${M}|${EL}`, ...L);
        return E[M];
      },
      set() {
        throw Error("You may not add new properties to elements");
  ...
  var QM = new Proxy(
    ...
    {
      get(E, M) {
        if (E[M] === undefined) E[M] = (...L) => JM(`${M}|${eM}`, ...L);
        return E[M];
      },
      set() {
        throw Error("You may not add new properties to elements");
  ...
  function tL(E, M, L = "part") {
    ...
    for (let H of Array.from(E.querySelectorAll(f))) {
      if (LL.has(H)) continue;
      let $ = H.getAttribute(`data-${L}`);
      if ($ == null) continue;
      let J = M[$];
      if (J == null) continue;
      LL.add(H);
      for (let Q of Object.keys(J)) BE(H, Q, J[Q]);
    }
  }
  var aL = [
    "sort",
  ...
  var a = (E = "", M = "") => {
    if (E === "") return M;
    else if (M.match(/^\d+$/) !== null || M.includes("=")) return `${E}[${M}]`;
    else return `${E}.${M}`;
  };
  var _E = {};
  function YM(E, M) {
  ...
  var FM = (E, M, L) => {
    for (let f = 0; f < E.length; f++) if (`${V(E[f], M)}` === `${L}`) return f;
    return -1;
  };
  var Lf = (E, M) => ({
    listFind(L, f) {
      if (L instanceof Element) {
        let J = L;
        while (J && !J[o] && J.parentElement) J = J.parentElement;
        let Q = J?.[o];
        if (Q == null) return;
        let Z = M.indexOf(Q);
    ...
    listUpdate(L, f) {
      ...
      if (J !== -1) {
        let Q = x[E][J];
        for (let Z of Object.keys(f)) Q[Z] = f[Z];
        return Q;
      }
      ...
      switch (f) {
        ...
        case "listBinding":
          return (H = ({ span: J }) => J({ bindText: "^" }), $ = {}) => {
            let J = $.virtual?.itemsPerRow ?? 1,
              Q = [];
            for (let Z = 0; Z < J; Z++) Q.push(H(I, YL(), Z));
        ...
        case "take":
          return (...H) => {
            let $ = H[H.length - 1],
              Q = H.slice(0, -1).map((Z) => (typeof Z === "string" ? Z : Z[m]));
  ...
  var e = (E, M) => ({
    get(L, f) {
      if ((f === "tosi" || f === jE) && M) return qE(E, L);
      if ($L(L)) {
        let Z = () => V(T, E);
        switch (f) {
          case "valueOf":
          ...
          case Symbol.toPrimitive:
            return (z) => {
              let G = Z();
              if (z === "number") return Number(G);
              if (z === "string") return String(G);
              return G;
            };
          case "toString":
            return () => String(Z());
        }
        if (ZL.has(f)) return qE(E, L)[f];
        let Y = QL.get(f);
        if (Y !== undefined) return ff(), qE(E, L)[Y];
        let F = Z();
        if (F != null) {
          let z = Object(F);
          if (f in z) {
      ...
      if (M && !(f in L) && ZL.has(f)) return qE(E, L)[f];
      let H = QL.get(f);
      if (H !== undefined) return qE(E, L)[H];
      if (typeof f === "symbol") return L[f];
      let $ = Object.getOwnPropertyDescriptor(L, f);
      if ($ && !$.configurable && !$.writable && "value" in $) return $.value;
      let J = f,
        Q =
          J.match(/^([^.[]+)\.(.+)$/) ??
          J.match(/^([^\]]+)(\[.+)/) ??
      ...
      if (
        (!Array.isArray(L) && L[J] !== undefined) ||
        (Array.isArray(L) && J.includes("="))
      ) {
        let Z;
        if (J.includes("=")) {
          let [Y, F] = J.split("=");
        ...
        return typeof Z === "function"
          ? (...Y) => {
              let F = Y.map((G) => C(G)),
                z = Z.apply(L, F);
              if (aL.includes(J)) N(E);
              if (z != null && typeof z === "object") {
                if (J === "find" || J === "findLast" || J === "at") {
                  let G = L.indexOf(z);
                  if (G !== -1) return new Proxy(z, e(a(E, String(G)), M));
                }
              }
              return z;
            }
          : typeof Z === "object"
          ? new Proxy(C(Z), e(a(E, J), M))
          : M
          ? YM(Z, a(E, J))
          : Z;
      } else {
        let Z = L[J];
        if (Z !== null && typeof Z === "object") Z = C(Z);
        return M ? YM(Z, a(E, J)) : Z;
      }
    },
    set(L, f, H) {
      if (((H = C(H)), H !== null && typeof H === "object"))
        if (Array.isArray(H)) for (let Z = 0; Z < H.length; Z++) H[Z] = C(H[Z]);
        else for (let Z of Object.keys(H)) H[Z] = C(H[Z]);
      let J =
        f === r ||
        f === "xinValue" ||
        f === "tosiValue" ||
        (f === "value" && ($L(L) || M))
          ? E
          : a(E, f);
      if (eL && !Mf(J)) throw Error(`setting invalid path ${J}`);
      if (C(k[J]) !== H && l(T, J, H)) N(J);
      return true;
    },
  });
  var KE = (E, M) => {
    ...
    if (typeof L !== "function")
      throw Error(
        `observe expects a function or path to a function, ${M} is neither`
      );
    return i(E, L);
  };
  ...
  function zL(E, M) {
    ...
    if (E.matches(p)) L.unshift(E);
    for (let f of L) {
      let H = b.get(f);
      for (let $ of H) {
        if ($.path.startsWith("^")) $.path = `${M}${$.path.substring(1)}`;
        if ($.binding.toDOM != null) $.binding.toDOM(f, k[$.path]);
      }
    }
  }
  ...
  class FL {
    ...
    constructor(E, M, L = {}) {
      if (
        ...
        let Q = j(M);
        if (Q != null) SM(Q, L.idPath);
      }
      let f = L.virtual?.itemsPerRow ?? 1,
        H = Array.from(E.children).find(
          (Q) => Q instanceof HTMLTemplateElement
        ),
        $ = null;
      if (H != null) {
        let Q = f;
        if (H.content.children.length < 1 || H.content.children.length !== Q)
          throw Error(
            `ListBinding expects a template with exactly ${Q} child element(s)`
          );
        (this.templates = Array.from(H.content.children).map((Z) => LE(Z))),
      ...
      if (
        ((this.isNamespaced =
          ...
          J === "http://www.w3.org/1998/Math/MathML"),
        this.isNamespaced)
      )
        ...
        this.boundElement.insertBefore(this.listTop, $),
          this.boundElement.insertBefore(this.listBottom, $);
      else
      ...
      if (f > 1) {
        ...
        if (Q != null)
          Q.setProperty("--tosi-columns", String(f)),
            (Q.display = Q.display || "grid"),
            (Q.gridTemplateColumns =
              Q.gridTemplateColumns || "repeat(var(--tosi-columns), 1fr)");
      ...
      if (
        ((this.boundElement[QE] = this), this.isNamespaced && L.virtual != null)
      )
        console.warn(
    ...
    visibleSlice() {
      ...
      if (E != null && this.boundElement instanceof HTMLElement) {
        ...
        if (Q) {
          ...
          (Z = this.boundElement.offsetHeight),
            (Y = this.boundElement.scrollTop);
        let F =
            E.width != null
              ? Math.max(1, Math.floor(J / E.width))
              : E.visibleColumns ?? 1,
          z = Math.ceil(M.length / F);
        if (E.minHeight != null) {
          let G = E.minHeight,
            B = Math.ceil(Z / G) + (E.rowChunkSize || 1),
            W = F * B,
            D = z * G,
            w = Math.max(0, D - Z),
            K = w > 0 ? Math.min(1, Math.max(0, Y / w)) : 0,
            g = Math.max(0, z - B + 1),
            y = K * g,
            S = Math.floor(y);
          if (E.rowChunkSize) S -= S % E.rowChunkSize;
          return (
            (L = S * F),
            (f = L + W - 1),
            (H = Y),
          ...
          let G = Math.ceil(Z / E.height) + (E.rowChunkSize || 1),
            B = F * G,
            W = Math.floor(Y / E.height);
          if (W > z - G + 1) W = Math.max(0, z - G + 1);
          if (E.rowChunkSize) W -= W % E.rowChunkSize;
          (L = W * F),
            (f = L + B - 1),
            (H = W * E.height),
            ($ = Math.max((z - G) * E.height - H, 0));
        }
      }
      return {
        items: M,
    ...
    needle;
    filter = FE((E) => {
      if (this.needle !== E) (this.needle = E), this.update(this.array);
    }, $f);
    update(E, M) {
      if (E == null) E = [];
      if (((this.array = E), !M)) this._filteredCache = undefined;
      let { hiddenProp: L, visibleProp: f } = this.options,
        H = j(E),
        $ = this.visibleSlice();
      if (
      ...
      let J = this._previousSlice,
        { firstItem: Q, lastItem: Z, topBuffer: Y, bottomBuffer: F } = $,
        z =
          L === undefined &&
          f === undefined &&
          M === true &&
      ...
      let G = 0,
        B = 0,
        W = 0,
        { idPath: D } = this.options,
        w = this.options.virtual?.itemsPerRow ?? 1,
        K;
      if (D != null) {
        K = new Set();
        for (let q = Q; q <= Z; q++) {
          let U = $.items[q];
          if (U !== undefined) K.add(String(U[D]));
        }
      }
      let g = new Set();
      for (let q of Array.from(this.boundElement.children)) {
        if (q === this.listTop || q === this.listBottom) continue;
        let U = q[o];
        if (U == null) continue;
        else if (U === true) q.remove(), G++;
        else {
          if (g.has(U)) continue;
          let O;
          if (D != null) O = !K.has(String(U[D]));
          else {
          ...
          if (O) {
            ...
            if (_ != null) for (let A of _) A.remove();
            else q.remove();
      ...
      if (this.listTop != null && this.listBottom != null)
        (this.listTop.style.height = String(Y) + "px"),
      ...
      for (let q = Q; q <= Z; q++) {
        ...
        if (O == null) {
          ...
          for (let A of O) A[o] = typeof U === "object" ? _ : true;
          for (let A of O)
            if (this.listBottom != null)
              this.boundElement.insertBefore(A, this.listBottom);
            else this.boundElement.append(A);
          if (D != null) {
            let A = U[D],
              d = `${H}[${D}=${A}]`;
            for (let jL of O) zL(jL, d);
            this.idToElement.set(String(A), O);
          } else {
            let A = `${H}[${q}]`;
            for (let d of O) zL(d, A);
          }
        }
          ...
          if (w > 1)
            for (let A = 0; A < O.length; A++) {
              ...
              d.setAttribute("role", "gridcell"),
                d.setAttribute("aria-rowindex", _),
                d.setAttribute("aria-colindex", String(A + 1));
            }
          else
            for (let A of O)
              A.setAttribute("role", "listitem"),
                A.setAttribute("aria-rowindex", _);
        }
        y.push(...O);
      }
      let S = null;
      for (let q of y) {
        if (q.previousElementSibling !== S)
          ...
          else this.boundElement.append(q);
        S = q;
      }
      if ($.interpolation != null) this._updateInterpolatedBuffers($);
      if (fE.perf)
    ...
    _updateInterpolatedBuffers(E) {
      ...
      for (let z of Array.from(this.boundElement.children)) {
        if (z === this.listTop || z === this.listBottom) continue;
        Q += z.offsetHeight || J;
      }
      let Z = f,
        Y = f + H - Q,
        F = Math.max(0, M * Y + (1 - M) * Z - (L % 1) * J);
      if (this.listTop != null && this.listBottom != null)
        (this.listTop.style.height = String(F) + "px"),
          (this.listBottom.style.height =
            String(Math.max(0, $ - F - Q)) + "px");
    }
  }
  var DE = (E, M, L) => {
    let f = E[QE];
    if (M && f === undefined) (f = new FL(E, M, L)), (E[QE] = f);
    return f;
  };
  var XM = (E) => {
    let M;
    while (!(M = E[o]) && E && E.parentElement) E = E.parentElement;
  ...
  var Zf = (E) => {
    ...
    if (f > -1) return L.array.splice(f, 1), true;
    return false;
  };
  ...
  var Yf = (E, M, L = {}) => {
    ...
    let { position: H = "middle", behavior: $ = "smooth" } = L,
      J = f.filteredArray(),
      Q = C(M) ?? M,
      Z = J.indexOf(Q);
    if (Z === -1)
      return (
        console.error(
          "scrollListItemIntoView failed, item not found in list",
          M
        ),
        false
      );
    ...
    if (Y != null && E instanceof HTMLElement) {
      let F =
          Y.width != null
            ? Math.max(1, Math.floor(E.offsetWidth / Y.width))
            : Y.visibleColumns ?? 1,
        z = Math.floor(Z / F),
        G = Y.minHeight ?? Y.height,
        B = Math.ceil(J.length / F),
        W = Y.scrollContainer === "window",
        D = W ? window.innerHeight : E.offsetHeight,
        w;
      if (Y.minHeight != null) {
        let K = Math.ceil(D / G) + (Y.rowChunkSize || 1),
          g = B * G,
          y = Math.max(0, g - D),
          S = Math.max(1, B - K + 1),
          q = z / S;
        switch (H) {
          ...
          case "nearest": {
            let U = W
                ? Math.max(0, -E.getBoundingClientRect().top)
                : E.scrollTop,
              O = y > 0 ? U / y : 0,
              _ = Math.floor(O * S);
            if (z < _) w = q * y;
            else if (z >= _ + K) w = Math.max(0, ((z - K + 1) / S) * y);
            else return true;
            break;
          }
          default: {
        ...
        switch (H) {
          ...
          case "nearest": {
            let g = W
              ...
              : E.scrollTop;
            if (K < g) w = K;
            else if (K + Y.height > g + D) w = K - D + Y.height;
            else return true;
            break;
          }
          default:
      ...
      } else E.scrollTo({ top: w, behavior: $ });
    } else {
  ...
  var WM = (E, M) => {
    ...
    for (let f of L) {
      ...
      if (Q != null) {
        ...
        if (M == null || J.startsWith(M)) Q(E, GE()[J], $);
      }
    }
  };
  if (XL != null)
      ...
      for (let f of L) {
        ...
        if (J != null) {
          ...
          try {
            ...
            throw (
              (console.error("Cannot get value from", M, "via", f),
  ...
  if (globalThis.document != null)
    OE.body.addEventListener("change", GL, true),
      OE.body.addEventListener("input", GL, true);
  function zf(E, M, L, f) {
    let { paths: H, transform: $ } = M,
      { toDOM: J } = L;
    if (J == null) return E;
    let Q = null,
      Z = {
        toDOM(F, z, G) {
          let B = GE(),
            W = H.map((w) => B[w]);
          if (Q !== null && W.every((w, K) => w === Q[K])) return;
          Q = W;
          let D = $(...W);
          J(F, D, G);
        },
    ...
    E.classList?.add(wE);
    let Y = b.get(E);
    if (Y == null) (Y = []), b.set(E, Y);
    for (let F of H) Y.push({ path: F, binding: Z, options: f });
    if (!H[0].startsWith("^")) N(H[0]);
    return E;
  }
  function v(E, M, L, f) {
    if (E instanceof DocumentFragment)
      throw Error("bind cannot bind to a DocumentFragment");
    if (M != null && typeof M === "object" && M[ME]) return zf(E, M, L, f);
    let H;
    if (typeof M === "object" && M[m] === undefined && f === undefined) {
      let { value: Q } = M;
      (H = typeof Q === "string" ? Q : Q[m]), (f = M), delete f.value;
    } else H = typeof M === "string" ? M : M[m];
    if (H == null) throw Error("bind requires a path or object with xin Proxy");
    let { toDOM: $ } = L;
    E.classList?.add(wE);
    let J = b.get(E);
    if (J == null) (J = []), b.set(E, J);
    if (
      (J.push({ path: H, binding: L, options: f }),
      $ != null && !H.startsWith("^"))
    )
      N(H);
  ...
  var Ff = (E) => {
    ...
    while (!L && M != null) {
      ...
      for (let Q of J) {
        if (typeof Q === "function") Q(f);
        else {
          let Z = GE()[Q];
          if (typeof Z === "function") Z(f);
          else throw Error(`no event handler found at path ${Q}`);
        }
        if (L) continue;
      }
  ...
  function XE(E, M, L) {
    let f = EE.get(E);
    if ((E.classList.add(vE), f == null)) (f = {}), EE.set(E, f);
    if (!f[M]) f[M] = new Set();
    ...
    if (J && M !== "")
      try {
        if (!new RegExp(`^(?:${J})$`).test(M))
          (L.patternMismatch = true),
            (f = "Please match the requested format.");
      } catch {}
    if (Object.keys(L).length > 0) E.internals.setValidity(L, f, E);
    else E.internals.setValidity({});
  }
  var Xf = 0;
  function DM() {
      ...
      KM = new MutationObserver((E) => {
        ...
        for (let L of E)
          if (L.type === "attributes" && L.target instanceof u) {
            let f = L.target,
              H = EM(L.attributeName);
            if (f._legacyTrackedAttrs?.has(H)) M.add(f);
          }
        for (let L of M) L.queueRender(false);
      });
    return KM;
  }
  var AE = {};
...
` +
  ...
  function Uf(E) {
    if (AE[E]) document.head.append(I.style({ id: E + "-component" }, AE[E]));
    delete AE[E];
  }
  ...
  class u extends HTMLElement {
    ...
    setCustomValidity(E) {
      if (this.internals)
        if (E) this.internals.setValidity({ customError: true }, E);
    ...
    static get observedAttributes() {
      ...
      if (E) return ["hidden", ...Object.keys(E).map(P)];
      return ["hidden"];
    }
    instanceId;
    styleNode;
      ...
      if (!Object.prototype.hasOwnProperty.call(M, "_elementCreator")) {
        ...
        if (L == null)
          if (typeof M.name === "string" && M.name !== "") {
            if (((L = P(M.name)), L.startsWith("-"))) L = L.slice(1);
          } else L = DM();
        if (customElements.get(L) != null)
          console.warn(`${L} is already defined`);
        if (L.match(/\w+(-\w+)+/) == null)
          console.warn(`${L} is not a legal tag for a custom-element`),
            (L = DM());
        while (customElements.get(L) !== undefined) L = DM();
        M._tagName = L;
        let f = E.styleSpec ?? M.lightStyleSpec;
        if (f !== undefined) Wf(L, f);
        let H = E.extends ?? M.extends,
          $ = H ? { extends: H } : undefined;
    ...
    initAttributes(...E) {
      if (
        ...
        !this._legacyTrackedAttrs)
      )
        this._legacyTrackedAttrs = new Set();
      for (let H of E) this._legacyTrackedAttrs.add(H);
      ...
      E.forEach((H) => {
        ...
        Object.defineProperty(this, H, {
          enumerable: false,
          get() {
            if (typeof L[H] === "boolean") return this.hasAttribute($);
            else if (this.hasAttribute($))
              return typeof L[H] === "number"
                ? parseFloat(this.getAttribute($))
                : this.getAttribute($);
            else if (f[H] !== undefined) return f[H];
            else return L[H];
          },
          set(J) {
            if (typeof L[H] === "boolean") {
              if (J !== this[H]) {
                if (J) this.setAttribute($, "");
    ...
    initValue() {
      ...
      if (E === undefined || E.get !== undefined || E.set !== undefined) return;
      let M = this.hasAttribute("value")
          ...
          {},
          {
            get(M, L) {
              if (M[L] === undefined) {
                ...
                if (f == null) f = E.querySelector(L);
                if (f == null) throw Error(`elementRef "${L}" does not exist!`);
    ...
    attributeChangedCallback(E, M, L) {
      let f = EM(E);
      if (!this._legacyTrackedAttrs?.has(f)) this.queueRender(false);
    }
    constructor() {
      super();
      if (
      ...
      if (E) this._setupAttributeAccessors(E);
      (this.instanceId = `${this.tagName.toLocaleLowerCase()}-${UL}`),
        (this._value = n(this.defaultValue));
    }
    _setupAttributeAccessors(E) {
      if (!this._attrValues) this._attrValues = new Map();
      for (let M of Object.keys(E)) {
        ...
        if (typeof f === "object" && f !== null) {
          console.warn(
            `${this.tagName}: initAttributes.${M} is an object. Use a regular property instead.`
          );
          continue;
        }
        let H = this,
        ...
        Object.defineProperty(this, M, {
          enumerable: false,
          get: () => {
            if (typeof f === "boolean") return this.hasAttribute(L);
            else if (this.hasAttribute(L))
              return typeof f === "number"
                ? parseFloat(this.getAttribute(L))
                : this.getAttribute(L);
            else if (this._attrValues.has(M)) return this._attrValues.get(M);
            else return f;
          },
          set: (J) => {
            if (typeof f === "boolean") {
              if (J !== this[M]) {
                if (J) this.setAttribute(L, "");
                else this.removeAttribute(L);
                this.queueRender();
              }
    ...
    formDisabledCallback(E) {
      if (E) this.setAttribute("disabled", "");
      else this.removeAttribute("disabled");
    }
    formStateRestoreCallback(E) {
      if (this.value !== undefined && typeof E === "string") this.value = E;
    }
    _changeQueued = false;
    _renderQueued = false;
    queueRender(E = false) {
      if (!this._hydrated) return;
      if (!this._changeQueued) this._changeQueued = E;
      if (!this._renderQueued)
        (this._renderQueued = true),
          requestAnimationFrame(() => {
            if (this._changeQueued) {
              if (
                (aE(this, "change"), this.internals && this.value !== undefined)
              )
    ...
    hydrate() {
      if (!this._hydrated) {
        this.initValue();
        let E = typeof this.content !== "function",
          M =
            typeof this.content === "function" ? this.content(I) : this.content;
        if (Array.isArray(M)) {
          let $ = {};
          M = M.filter((J) => {
            if (
          ...
          for (let J of Object.keys($)) BE(this, J, $[J]);
        }
        let L = this.constructor,
          f = L.shadowStyleSpec ?? L.styleSpec;
        ...
        let { styleNode: H } = L;
        if (f) (H = L.styleNode = I.style($E(f))), delete L.styleNode;
        if (this.styleNode)
          console.warn(
            this,
            "styleNode is deprecated, use static shadowStyleSpec instead"
        ...
        if (H) {
          ...
          if (J.length > 0) J.forEach(SE.replaceSlot);
          if ($.length > 0) {
            let Q = { "": this };
            Array.from(this.querySelectorAll("tosi-slot,xin-slot")).forEach(
              ...
              $.forEach((Z) => {
                let Y = Q[""],
                  F = Z instanceof Element ? Q[Z.slot] : Y;
  ...
  class SE extends u {
    ...
    static replaceSlot(E) {
      ...
      if (E.name !== "") M.setAttribute("name", E.name);
      E.replaceWith(M);
    }
  }
  ...
  var qf = (E = () => true) => {
    ...
    if (M != null) {
      ...
      for (let H of Object.keys(f).filter(E))
        if (k[H] !== undefined) Object.assign(k[H], f[H]);
        else k[H] = f[H];
    }
    let L = UE(() => {
      let f = {},
        H = C(k);
      for (let $ of Object.keys(H).filter(E)) f[$] = H[$];
      localStorage.setItem("xin-state", JSON.stringify(f)),
  ...
  var wf = null;
  function KL() {
    if (NE != null) return Promise.resolve(NE);
  ...
  function OL(E) {
    for (let M of OM) if (E === M || E.startsWith(M + ".")) return M;
    return;
  }
  function Tf(E) {
    for (let M of AM) if (E === M || E.startsWith(M + ".")) return true;
    return false;
  }
  function Rf(E, M) {
    AM.add(E),
  ...
  function kf() {
    ...
    return (
      ...
      (ZE = new BroadcastChannel(Of)),
      (ZE.onmessage = (E) => {
        let M = E.data;
        if (!Vf(M)) return;
        if (M.origin === CM) return;
        if (OL(M.path) === undefined) return;
        Rf(M.path, M.value);
      }),
      ZE
    );
  ...
  async function _f(...E) {
    if (typeof BroadcastChannel > "u") return { restored: [] };
    kf();
    let M = [],
      L = qL();
    for (let f of E) {
      ...
      if (OM.has(H)) continue;
      OM.add(H);
      let $ = await L.get(H);
      if ($ !== undefined) l(T, H, $), N(H), M.push(f);
      else {
        let J = V(T, H);
        await L.set(H, J);
      }
      i(
        (J) => J === H || J.startsWith(H + "."),
        (J) => {
          if (Tf(J)) return;
          let Q = OL(J);
  ...
  var wM = new Set();
  function AL(E, M) {
    for (let L of E) if (M === L || M.startsWith(L + ".")) return L;
    return;
  }
  function Sf(E) {
    for (let M of wM) if (E === M || E.startsWith(M + ".")) return true;
    return false;
  }
  function Nf(E, M) {
    wM.add(E),
  ...
  async function xf(E, M, ...L) {
    ...
    E.onReceive((Z) => {
      for (let Y of Z) {
        if (AL(f, Y.path) === undefined) continue;
        Nf(Y.path, Y.value);
      }
    });
    for (let Z of L) {
      ...
      f.add(Y);
      let F = i(
        (z) => z === Y || z.startsWith(Y + "."),
        (z) => {
          if (Sf(z)) return;
          if (AL(f, z) === undefined) return;
          let G = V(T, z);
  ...
  function yE(E) {
    return (
      R("boxedProxy", "boxedProxy is deprecated, please use tosi() instead"),
      xE(E)
    );
  }
  ...
  class bE extends u {
    ...
    blueprintLoaded = (E) => {};
    async packaged() {
      let { tag: E, src: M, property: L } = this,
        f = `${E}.${L}:${M}`;
      if (!this.loaded) {
        if (TM[f] === undefined)
          TM[f] = bf(M).then((H) => {
        ...
        else console.log(`using cached ${E} with signature ${f}`);
  ...
  class RM extends u {
    ...
    allLoaded = () => {};
    async load() {
      let M = Array.from(this.querySelectorAll("tosi-blueprint, xin-blueprint"))
        .filter((L) => L.src)
        .map((L) => L.packaged());
  ...
  class wL extends u {
    ...
    async load() {
      let M = Array.from(this.querySelectorAll("xin-blueprint"))
        .filter((L) => L.src)
        .map((L) => L.packaged());
      await Promise.all(M), this.allLoaded();
    }
  ...
  function interpolateThemeValue(from, to, t2) {
    ...
    if (aNums.length > 0 && aNums.length === bNums.length) {
      ...
      for (let i2 = 0; i2 < aNums.length; i2++) {
        ...
        const v2 =
          parseFloat(am[0]) + (parseFloat(bm[0]) - parseFloat(am[0])) * t2;
  ...
  class TosiProduct extends u {
    ...
    _relayout() {
      ...
      for (const child of Array.from(this.children)) {
        ...
        const isSection =
          child.tagName.toLowerCase() === "tosi-product-section";
    ...
    _update() {
      ...
      for (let i2 = 0; i2 < this._items.length; i2++) {
        ...
        let progress;
        if (i2 < activeIdx) progress = 1;
        else if (i2 > activeIdx) progress = 0;
...
` +
  ...
  var interpolateStrings = (a2, b2, t2) => {
    ...
    const isColor2 = (s2) =>
      ...
      s2.startsWith("rgb") ||
      s2.startsWith("hsl") ||
      ["red", "blue", "white", "black", "transparent"].includes(s2);
  ...
  class TosiInterpolator extends u {
    ...
    setScrollProgress(progress) {
      ...
      if (progress <= wp1.progress) {
        ...
        for (let i2 = 0; i2 < waypoints.length - 1; i2++) {
          if (
            progress >= waypoints[i2].progress &&
            progress <= waypoints[i2 + 1].progress
          ) {
            wp1 = waypoints[i2];
            wp2 = waypoints[i2 + 1];
            const rawT =
      ...
      .map((wp) => {
        ...
        for (const attr of Array.from(wp.attributes)) {
          ...
          if (Number.isFinite(val)) {
            const key = attr.name.replace(/-([a-z])/g, (_, c2) =>
              c2.toUpperCase()
            );
            result[key] = val;
          }
  ...
  class TosiScrollCamera extends u {
    ...
    setScrollProgress(progress) {
      ...
      if ("alpha" in v2 && camera.alpha !== undefined) camera.alpha = v2.alpha;
      if ("beta" in v2 && camera.beta !== undefined) camera.beta = v2.beta;
      ...
      if (camera.target && typeof camera.target.copyFromFloats === "function") {
        if ("targetX" in v2 || "targetY" in v2 || "targetZ" in v2) {
          camera.target.copyFromFloats(
            v2.targetX ?? camera.target.x,
            v2.targetY ?? camera.target.y,
            v2.targetZ ?? camera.target.z
          );
        }
      }
      if (camera.position) {
        if ("x" in v2) camera.position.x = v2.x;
        if ("y" in v2) camera.position.y = v2.y;
        if ("z" in v2) camera.position.z = v2.z;
      }
      if ("fov" in v2 && camera.fov !== undefined) camera.fov = v2.fov;
    }
  }
  ...
  var E = { exec: () => null };
  function d(l2, e2 = "") {
    let t2 = typeof l2 == "string" ? l2 : l2.source,
      n2 = {
        replace: (r2, i2) => {
          let s2 = typeof i2 == "string" ? i2 : i2.source;
          return (
            (s2 = s2.replace(m2.caret, "$1")), (t2 = t2.replace(r2, s2)), n2
          );
        },
  ...
  var m2 = {
    ...
    fencesBeginRegex: (l2) =>
      new RegExp(`^ {0,${Math.min(3, l2 - 1)}}(?:\`\`\`|~~~)`),
    headingBeginRegex: (l2) => new RegExp(`^ {0,${Math.min(3, l2 - 1)}}#`),
    htmlBeginRegex: (l2) =>
      new RegExp(`^ {0,${Math.min(3, l2 - 1)}}<(?:[a-z].*>|!--)`, "i"),
  };
  var Re = /^(?:[ \t]*(?:\n|$))+/;
  var Te = /^((?: {4}| {0,3}\t)[^\n]+(?:\n(?:[ \t]*(?:\n|$))*)?)+/;
  var Oe =
  ...
  var W = /[\s\p{P}\p{S}]/u;
  var ue = /[^\s\p{P}\p{S}]/u;
  var Ce = d(/^((?![*_])punctSpace)/, "u")
    .replace(/punctSpace/g, W)
    .getRegex();
  var pe = /(?!~)[\p{P}\p{S}]/u;
  var Be = /(?!~)[\s\p{P}\p{S}]/u;
  var qe = /(?:[^\s\p{P}\p{S}]|~)/u;
  var ve = d(/link|precode-code|html/, "g")
    .replace(
      "link",
      /\[(?:[^\[\]`]|(?<a>`+)[^`]+\k<a>(?!`))*?\]\((?:\\[\s\S]|[^\\\(\)]|\((?:\\[\s\S]|[^\\\(\)])*\))*\)/
    )
  ...
  var Fe = d(/\\(punct)/, "gu")
    .replace(/punct/g, D)
    .getRegex();
  var je = d(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/)
  ...
  function w(l2, e2) {
    if (e2) {
      if (m2.escapeTest.test(l2)) return l2.replace(m2.escapeReplace, ge);
  ...
  function V2(l2, e2) {
    let t2 = l2.replace(m2.findPipe, (i2, s2, a2) => {
      ...
      n2 = t2.split(m2.splitPipe),
      r2 = 0;
    if (
      (n2[0].trim() || n2.shift(),
      n2.length > 0 && !n2.at(-1)?.trim() && n2.pop(),
      e2)
    )
      if (n2.length > e2) n2.splice(e2);
  ...
  function me(l2, e2, t2, n2, r2) {
    let i2 = e2.href,
      s2 = e2.title || null,
      a2 = l2[1].replace(r2.other.outputLinkReplace, "$1");
    n2.state.inLink = true;
    let o2 = {
      type: l2[0].charAt(0) === "!" ? "image" : "link",
      raw: t2,
      href: i2,
      title: s2,
      text: a2,
      tokens: n2.inlineTokens(a2),
    };
...
`
    ...
    fences(e2) {
      ...
      if (t2) {
        let n2 = t2[0],
          r2 = Ye(n2, t2[3] || "", this.rules);
        return {
          type: "code",
          raw: n2,
          lang: t2[2]
            ? t2[2].trim().replace(this.rules.inline.anyPunctuation, "$1")
            : t2[2],
          text: r2,
        };
      }
      ...
      if (t2) {
        ...
        if (this.rules.other.endingHash.test(n2)) {
          ...
          (this.options.pedantic ||
            !r2 ||
            this.rules.other.endingSpaceChar.test(r2)) &&
            (n2 = r2.trim());
        }
        return {
          type: "heading",
...
${c2}`
          ...
          if (
            ...
            this.lexer.blockTokens(c2, s2, true),
...
`),
              O = this.blockquote(f);
            (s2[s2.length - 1] = O),
              (r2 = r2.substring(0, r2.length - R2.raw.length) + O.raw),
              (i2 = i2.substring(0, i2.length - R2.text.length) + O.text);
            break;
...
`),
              O = this.list(f);
            (s2[s2.length - 1] = O),
              (r2 = r2.substring(0, r2.length - h2.raw.length) + O.raw),
              (i2 = i2.substring(0, i2.length - R2.raw.length) + O.raw),
              (n2 = f.substring(s2.at(-1).raw.length).split(`
`));
            continue;
          }
        }
      ...
      if (t2) {
        let n2 = t2[1].trim(),
          ...
          i2 = {
            ...
            raw: "",
            ordered: r2,
            start: r2 ? +n2.slice(0, -1) : "",
            loose: false,
            items: [],
          };
        (n2 = r2 ? `\\d{1,9}\\${n2.slice(-1)}` : `\\${n2}`),
          this.options.pedantic && (n2 = r2 ? n2 : "[*+-]");
        let s2 = this.rules.other.listItemRegex(n2),
          a2 = false;
        for (; e2; ) {
          let p2 = false,
            u2 = "",
...
`,
          ...
          if (
            (this.options.pedantic
              ...
              : R2
              ? (f = t2[1].length + 1)
              : ((f = t2[2].search(this.rules.other.nonSpaceChar)),
                (f = f > 4 ? 1 : f),
                (c2 = g.slice(f)),
                (f += t2[1].length)),
            R2 &&
              this.rules.other.blankLine.test(h2) &&
              ((u2 +=
                h2 +
                `
`),
              (e2 = e2.substring(h2.length + 1)),
              (p2 = true)),
            !p2)
          ) {
              ...
              if (
                ...
                this.options.pedantic
                  ? ((h2 = h2.replace(
                    ...
                    (A = h2))
                  : (A = h2.replace(this.rules.other.tabCharGlobal, "    ")),
                te.test(h2) ||
                  ne.test(h2) ||
                  xe.test(h2) ||
                  H.test(h2) ||
...
`),
          ...
          i2.loose ||
            (a2
              ? (i2.loose = true)
              : this.rules.other.doubleBlankLine.test(u2) && (a2 = true));
          let O = null,
            Y;
          this.options.gfm &&
            ((O = this.rules.other.listIsTask.exec(c2)),
            O &&
              ((Y = O[0] !== "[ ] "),
              (c2 = c2.replace(this.rules.other.listReplaceTask, "")))),
            i2.items.push({
              type: "list_item",
              raw: u2,
              task: !!O,
        ...
        for (let p2 = 0; p2 < i2.items.length; p2++)
          if (
            ...
            (i2.items[p2].tokens = this.lexer.blockTokens(
              i2.items[p2].text,
              []
            )),
            !i2.loose)
          ) {
            let u2 = i2.items[p2].tokens.filter((g) => g.type === "space"),
              c2 =
                u2.length > 0 &&
                u2.some((g) => this.rules.other.anyLine.test(g.raw));
            i2.loose = c2;
          }
        if (i2.loose)
          for (let p2 = 0; p2 < i2.items.length; p2++)
    ...
    html(e2) {
      ...
      if (t2)
        return {
          ...
          block: true,
          raw: t2[0],
          pre: t2[1] === "pre" || t2[1] === "script" || t2[1] === "style",
          text: t2[0],
        };
    }
    def(e2) {
    ...
    table(e2) {
      ...
      let n2 = V2(t2[1]),
        r2 = t2[2].replace(this.rules.other.tableAlignChars, "").split("|"),
        i2 = t2[3]?.trim()
          ? t2[3].replace(this.rules.other.tableRowBlankLine, "").split(`
`)
          : [],
        s2 = { type: "table", raw: t2[0], header: [], align: [], rows: [] };
      if (n2.length === r2.length) {
        for (let a2 of r2)
          this.rules.other.tableAlignRight.test(a2)
            ? s2.align.push("right")
            : this.rules.other.tableAlignCenter.test(a2)
            ? s2.align.push("center")
            : this.rules.other.tableAlignLeft.test(a2)
            ? s2.align.push("left")
            : s2.align.push(null);
        for (let a2 = 0; a2 < n2.length; a2++)
          s2.header.push({
            text: n2[a2],
            tokens: this.lexer.inline(n2[a2]),
            header: true,
            align: s2.align[a2],
          });
        for (let a2 of i2)
          s2.rows.push(
            V2(a2, s2.header.length).map((o2, p2) => ({
              text: o2,
              tokens: this.lexer.inline(o2),
              header: false,
              align: s2.align[p2],
            }))
          );
    ...
    lheading(e2) {
      ...
      if (t2)
        return {
          type: "heading",
          raw: t2[0],
          depth: t2[2].charAt(0) === "=" ? 1 : 2,
          text: t2[1],
          tokens: this.lexer.inline(t2[1]),
        };
    }
    paragraph(e2) {
...
`
    ...
    tag(e2) {
      ...
      if (t2)
        return (
          !this.lexer.state.inLink && this.rules.other.startATag.test(t2[0])
            ? (this.lexer.state.inLink = true)
            : this.lexer.state.inLink &&
              this.rules.other.endATag.test(t2[0]) &&
              (this.lexer.state.inLink = false),
          !this.lexer.state.inRawBlock &&
          this.rules.other.startPreScriptTag.test(t2[0])
            ? (this.lexer.state.inRawBlock = true)
            : this.lexer.state.inRawBlock &&
              this.rules.other.endPreScriptTag.test(t2[0]) &&
    ...
    link(e2) {
      ...
      if (t2) {
        ...
        if (
          !this.options.pedantic &&
          this.rules.other.startAngleBracket.test(n2)
        ) {
          ...
          if (s2 > -1) {
            ...
            (t2[2] = t2[2].substring(0, s2)),
              (t2[0] = t2[0].substring(0, o2).trim()),
              (t2[3] = "");
          }
        }
        let r2 = t2[2],
        ...
        return (
          (r2 = r2.trim()),
          this.rules.other.startAngleBracket.test(r2) &&
            (this.options.pedantic && !this.rules.other.endAngleBracket.test(n2)
              ? (r2 = r2.slice(1))
              : (r2 = r2.slice(1, -1))),
          me(
            t2,
            {
              href: r2 && r2.replace(this.rules.inline.anyPunctuation, "$1"),
              title: i2 && i2.replace(this.rules.inline.anyPunctuation, "$1"),
            },
            t2[0],
            this.lexer,
            this.rules
    ...
    reflink(e2, t2) {
      let n2;
      if (
        (n2 = this.rules.inline.reflink.exec(e2)) ||
        (n2 = this.rules.inline.nolink.exec(e2))
      ) {
        let r2 = (n2[2] || n2[1]).replace(
            this.rules.other.multipleSpaceGlobal,
            " "
          ),
          i2 = t2[r2.toLowerCase()];
        if (!i2) {
    ...
    emStrong(e2, t2, n2 = "") {
      ...
      if (
        !(r2[1] || r2[2] || "") ||
        !n2 ||
        this.rules.inline.punctuation.exec(n2)
      ) {
        ...
        for (
          c2.lastIndex = 0, t2 = t2.slice(-1 * e2.length + s2);
          (r2 = c2.exec(t2)) != null;
          ...
          let g = [...r2[0]][0].length,
            h2 = e2.slice(0, s2 + r2.index + g + o2);
          if (Math.min(s2, o2) % 2) {
            let f = h2.slice(1, -1);
            return {
              type: "em",
          ...
          return {
            ...
            raw: h2,
            text: R2,
            tokens: this.lexer.inlineTokens(R2),
          };
        }
      }
    }
    codespan(e2) {
      ...
      if (t2) {
        let n2 = t2[2].replace(this.rules.other.newLineCharGlobal, " "),
          r2 = this.rules.other.nonSpaceChar.test(n2),
          i2 =
            this.rules.other.startingSpaceChar.test(n2) &&
            this.rules.other.endingSpaceChar.test(n2);
        return (
          r2 && i2 && (n2 = n2.substring(1, n2.length - 1)),
          { type: "codespan", raw: t2[0], text: n2 }
        );
      }
    }
    br(e2) {
    ...
    del(e2) {
      ...
      if (t2)
        return {
          ...
          raw: t2[0],
          text: t2[2],
          tokens: this.lexer.inlineTokens(t2[2]),
        };
    }
    autolink(e2) {
      ...
      if (t2) {
        ...
        return (
            ...
            : ((n2 = t2[1]), (r2 = n2)),
          {
            ...
            text: n2,
            href: r2,
            tokens: [{ type: "text", raw: n2, text: n2 }],
          }
        );
      }
    }
    url(e2) {
      ...
      if ((t2 = this.rules.inline.url.exec(e2))) {
        ...
        if (t2[2] === "@") (n2 = t2[0]), (r2 = "mailto:" + n2);
          ...
          do
            (i2 = t2[0]),
              (t2[0] = this.rules.inline._backpedal.exec(t2[0])?.[0] ?? "");
          while (i2 !== t2[0]);
          (n2 = t2[0]),
            t2[1] === "www." ? (r2 = "http://" + t2[0]) : (r2 = t2[0]);
        }
        return {
          type: "link",
          raw: t2[0],
  ...
  var x2 = class l2 {
    ...
    constructor(e2) {
      ...
      this.options.pedantic
        ? ((t2.block = C2.pedantic), (t2.inline = M.pedantic))
        : this.options.gfm &&
          ((t2.block = C2.gfm),
          this.options.breaks ? (t2.inline = M.breaks) : (t2.inline = M.gfm)),
        (this.tokenizer.rules = t2);
    }
    static get rules() {
...
`
    ...
    blockTokens(e2, t2 = [], n2 = false) {
      for (
        this.options.pedantic &&
        (e2 = e2.replace(m2.tabCharGlobal, "    ").replace(m2.spaceLine, ""));
        e2;

      ) {
        let r2;
        if (
          this.options.extensions?.block?.some((s2) =>
            (r2 = s2.call({ lexer: this }, e2, t2))
              ? ((e2 = e2.substring(r2.raw.length)), t2.push(r2), true)
              : false
          )
        )
          continue;
...
`)
        ...
        if ((r2 = this.tokenizer.code(e2))) {
          e2 = e2.substring(r2.raw.length);
          let s2 = t2.at(-1);
          s2?.type === "paragraph" || s2?.type === "text"
            ? ((s2.raw +=
                (s2.raw.endsWith(`
`)
                  ? ""
...
`) + r2.raw),
              (s2.text +=
                `
` + r2.text),
              (this.inlineQueue.at(-1).src = s2.text))
            : t2.push(r2);
          continue;
        }
...
` + r2.raw),
              (this.inlineQueue.at(-1).src = s2.text))
            : this.tokens.links[r2.tag] ||
              ((this.tokens.links[r2.tag] = { href: r2.href, title: r2.title }),
              t2.push(r2));
          continue;
        }
        ...
        if (this.options.extensions?.startBlock) {
          ...
          this.options.extensions.startBlock.forEach((p2) => {
            (o2 = p2.call({ lexer: this }, a2)),
              typeof o2 == "number" && o2 >= 0 && (s2 = Math.min(s2, o2));
          }),
            s2 < 1 / 0 && s2 >= 0 && (i2 = e2.substring(0, s2 + 1));
        }
        if (this.state.top && (r2 = this.tokenizer.paragraph(i2))) {
          let s2 = t2.at(-1);
          n2 && s2?.type === "paragraph"
            ? ((s2.raw +=
                (s2.raw.endsWith(`
`)
                  ? ""
...
` + r2.text),
    ...
    inlineTokens(e2, t2 = []) {
      ...
      if (this.tokens.links) {
        ...
        if (o2.length > 0)
          for (
            ;
            (r2 = this.tokenizer.rules.inline.reflinkSearch.exec(n2)) != null;

          )
            o2.includes(r2[0].slice(r2[0].lastIndexOf("[") + 1, -1)) &&
              (n2 =
                n2.slice(0, r2.index) +
                "[" +
                "a".repeat(r2[0].length - 2) +
                "]" +
                n2.slice(this.tokenizer.rules.inline.reflinkSearch.lastIndex));
      }
      for (
        ;
        (r2 = this.tokenizer.rules.inline.anyPunctuation.exec(n2)) != null;

      )
        n2 =
          n2.slice(0, r2.index) +
          "++" +
          n2.slice(this.tokenizer.rules.inline.anyPunctuation.lastIndex);
      let i2;
      for (; (r2 = this.tokenizer.rules.inline.blockSkip.exec(n2)) != null; )
        (i2 = r2[2] ? r2[2].length : 0),
          (n2 =
            n2.slice(0, r2.index + i2) +
            "[" +
            "a".repeat(r2[0].length - i2 - 2) +
            "]" +
            n2.slice(this.tokenizer.rules.inline.blockSkip.lastIndex));
      ...
      for (; e2; ) {
        ...
        let o2;
        if (
          this.options.extensions?.inline?.some((u2) =>
            (o2 = u2.call({ lexer: this }, e2, t2))
              ? ((e2 = e2.substring(o2.raw.length)), t2.push(o2), true)
              : false
          )
        )
          continue;
        ...
        if (this.options.extensions?.startInline) {
          ...
          this.options.extensions.startInline.forEach((h2) => {
            (g = h2.call({ lexer: this }, c2)),
              typeof g == "number" && g >= 0 && (u2 = Math.min(u2, g));
          }),
            u2 < 1 / 0 && u2 >= 0 && (p2 = e2.substring(0, u2 + 1));
        }
        if ((o2 = this.tokenizer.inlineText(p2))) {
          (e2 = e2.substring(o2.raw.length)),
            o2.raw.slice(-1) !== "_" && (a2 = o2.raw.slice(-1)),
            (s2 = true);
          let u2 = t2.at(-1);
          u2?.type === "text"
            ? ((u2.raw += o2.raw), (u2.text += o2.text))
  ...
  var P2 = class {
    ...
    code({ text: e2, lang: t2, escaped: n2 }) {
      let r2 = (t2 || "").match(m2.notSpaceStart)?.[0],
        i2 =
          e2.replace(m2.endingNewline, "") +
          `
`;
      return r2
...
`
    ...
    listitem(e2) {
      ...
      if (e2.task) {
        ...
        e2.loose
          ? e2.tokens[0]?.type === "paragraph"
            ? ((e2.tokens[0].text = n2 + " " + e2.tokens[0].text),
              e2.tokens[0].tokens &&
                e2.tokens[0].tokens.length > 0 &&
                e2.tokens[0].tokens[0].type === "text" &&
                ((e2.tokens[0].tokens[0].text =
                  n2 + " " + w(e2.tokens[0].tokens[0].text)),
                (e2.tokens[0].tokens[0].escaped = true)))
            : e2.tokens.unshift({
                type: "text",
                raw: n2 + " ",
                text: n2 + " ",
      ...
      return (
        (t2 += this.parser.parse(e2.tokens, !!e2.loose)),
        `<li>${t2}</li>
`
      );
    }
    checkbox({ checked: e2 }) {
      return (
        "<input " + (e2 ? 'checked="" ' : "") + 'disabled="" type="checkbox">'
      );
    }
    paragraph({ tokens: e2 }) {
...
`;
    ...
    tablecell(e2) {
      let t2 = this.parser.parseInline(e2.tokens),
        n2 = e2.header ? "th" : "td";
      return (
        (e2.align ? `<${n2} align="${e2.align}">` : `<${n2}>`) +
        t2 +
        `</${n2}>
`
      );
    ...
    link({ href: e2, title: t2, tokens: n2 }) {
      let r2 = this.parser.parseInline(n2),
        i2 = J(e2);
      if (i2 === null) return r2;
      e2 = i2;
      let s2 = '<a href="' + e2 + '"';
      return (
        t2 && (s2 += ' title="' + w(t2) + '"'), (s2 += ">" + r2 + "</a>"), s2
      );
    }
    ...
    text(e2) {
      return "tokens" in e2 && e2.tokens
        ? this.parser.parseInline(e2.tokens)
        : "escaped" in e2 && e2.escaped
        ? e2.text
        : w(e2.text);
    }
  };
  ...
  var b2 = class l3 {
    ...
    constructor(e2) {
      (this.options = e2 || T2),
        (this.options.renderer = this.options.renderer || new P2()),
    ...
    parse(e2, t2 = true) {
      ...
      for (let r2 = 0; r2 < e2.length; r2++) {
        ...
        if (this.options.extensions?.renderers?.[i2.type]) {
          let a2 = i2,
            o2 = this.options.extensions.renderers[a2.type].call(
              { parser: this },
              a2
            );
          if (
...
` + this.renderer.text(a2));
            t2
              ? (n2 += this.renderer.paragraph({
                  type: "paragraph",
                  raw: o2,
                  text: o2,
                  tokens: [{ type: "text", raw: o2, text: o2, escaped: true }],
                }))
              : (n2 += o2);
            continue;
          }
    ...
    parseInline(e2, t2 = this.renderer) {
      ...
      for (let r2 = 0; r2 < e2.length; r2++) {
        let i2 = e2[r2];
        if (this.options.extensions?.renderers?.[i2.type]) {
          let a2 = this.options.extensions.renderers[i2.type].call(
            { parser: this },
            i2
          );
          if (
  ...
  var B = class {
        ...
        switch (((n2 = n2.concat(t2.call(this, r2))), r2.type)) {
          case "table": {
            ...
            for (let s2 of i2.rows)
              for (let a2 of s2) n2 = n2.concat(this.walkTokens(a2.tokens, t2));
            break;
          }
          case "list": {
            let i2 = r2;
          ...
          default: {
            let i2 = r2;
            this.defaults.extensions?.childTokens?.[i2.type]
              ? this.defaults.extensions.childTokens[i2.type].forEach((s2) => {
    ...
    use(...e2) {
      ...
      return (
        e2.forEach((n2) => {
          let r2 = { ...n2 };
          if (
            ((r2.async = this.defaults.async || r2.async || false),
            n2.extensions &&
              (n2.extensions.forEach((i2) => {
                if (!i2.name) throw new Error("extension name required");
                if ("renderer" in i2) {
                  let s2 = t2.renderers[i2.name];
                  s2
                    ? (t2.renderers[i2.name] = function (...a2) {
                ...
                if ("tokenizer" in i2) {
                  if (
                    !i2.level ||
                    (i2.level !== "block" && i2.level !== "inline")
                  )
                    throw new Error(
                  ...
                  let s2 = t2[i2.level];
                  s2
                    ? s2.unshift(i2.tokenizer)
                    : (t2[i2.level] = [i2.tokenizer]),
                    i2.start &&
                      (i2.level === "block"
                        ? t2.startBlock
                          ? t2.startBlock.push(i2.start)
                          : (t2.startBlock = [i2.start])
                        : i2.level === "inline" &&
                          (t2.startInline
                            ? t2.startInline.push(i2.start)
                            : (t2.startInline = [i2.start])));
                }
                "childTokens" in i2 &&
                  i2.childTokens &&
                  (t2.childTokens[i2.name] = i2.childTokens);
              }),
              (r2.extensions = t2)),
            n2.renderer)
          ) {
          ...
          if (n2.hooks) {
            ...
            for (let s2 in n2.hooks) {
              ...
              let a2 = s2,
                o2 = n2.hooks[a2],
                p2 = i2[a2];
              S.passThroughHooks.has(s2)
                ? (i2[a2] = (u2) => {
                    if (
                      this.defaults.async &&
                      S.passThroughHooksRespectAsync.has(s2)
                    )
                      return (async () => {
          ...
          if (n2.walkTokens) {
            let i2 = this.defaults.walkTokens,
              s2 = n2.walkTokens;
            r2.walkTokens = function (a2) {
              let o2 = [];
              return (
                o2.push(s2.call(this, a2)),
                i2 && (o2 = o2.concat(i2.call(this, a2))),
                o2
              );
            };
          }
    ...
    parseMarkdown(e2) {
      return (n2, r2) => {
        let i2 = { ...r2 },
          s2 = { ...this.defaults, ...i2 },
          a2 = this.onError(!!s2.silent, !!s2.async);
          ...
          return a2(
            new Error(
              "marked(): input parameter is of type " +
                Object.prototype.toString.call(n2) +
                ", string expected"
            )
          );
        if (
          ...
          return (async () => {
            let o2 = s2.hooks ? await s2.hooks.preprocess(n2) : n2,
              u2 = await (s2.hooks
                ? await s2.hooks.provideLexer()
                : e2
                ? x2.lex
                : x2.lexInline)(o2, s2),
              c2 = s2.hooks ? await s2.hooks.processAllTokens(u2) : u2;
            s2.walkTokens &&
              (await Promise.all(this.walkTokens(c2, s2.walkTokens)));
            let h2 = await (s2.hooks
              ? await s2.hooks.provideParser()
              : e2
              ? b2.parse
              : b2.parseInline)(c2, s2);
            return s2.hooks ? await s2.hooks.postprocess(h2) : h2;
          })().catch(a2);
        try {
          s2.hooks && (n2 = s2.hooks.preprocess(n2));
          let p2 = (
            s2.hooks ? s2.hooks.provideLexer() : e2 ? x2.lex : x2.lexInline
          )(n2, s2);
          s2.hooks && (p2 = s2.hooks.processAllTokens(p2)),
            s2.walkTokens && this.walkTokens(p2, s2.walkTokens);
          let c2 = (
            s2.hooks ? s2.hooks.provideParser() : e2 ? b2.parse : b2.parseInline
          )(p2, s2);
...
Please report this to https://github.com/markedjs/marked.`),
  ...
  class _1 extends u {
    static set conditions(i2) {
      Object.assign(n2, i2);
      for (let l4 of [..._1.instances]) l4.queueRender();
    }
    ...
    render() {
      if (
        this.condition !== "" &&
        (this.not ? n2[this.condition] !== true : n2[this.condition] === true)
      )
        this.toggleAttribute("hidden", false);
      else this.toggleAttribute("hidden", true);
    }
  }
  ...
  function c2(i2) {
    if (t2[i2] === undefined) {
      ...
      document.head.append(l4),
        (t2[i2] = new Promise((s2) => {
          l4.onload = s2;
        }));
    }
    return t2[i2];
  ...
  var O1 = {
    ...
    arrowUpCircle:
      '<svg class="stroked" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"></circle><polyline points="16 12 12 8 8 12"></polyline><line x1="12" y1="16" x2="12" y2="8"></line></svg>',
    pauseCircle:
    ...
    arrowDown:
      '<svg class="stroked" viewBox="0 0 24 24"><line x1="12" y1="5" x2="12" y2="19"></line><polyline points="19 12 12 19 5 12"></polyline></svg>',
    figma:
    ...
    chevronsDown:
      '<svg class="stroked" viewBox="0 0 24 24"><polyline points="7 13 12 18 17 13"></polyline><polyline points="7 6 12 11 17 6"></polyline></svg>',
    ...
    cornerLeftUp:
      '<svg class="stroked" viewBox="0 0 24 24"><polyline points="14 9 9 4 4 9"></polyline><path d="M20 20h-7a4 4 0 0 1-4-4V4"></path></svg>',
    ...
    arrowUpLeft:
      '<svg class="stroked" viewBox="0 0 24 24"><line x1="17" y1="17" x2="7" y2="7"></line><polyline points="7 17 7 7 17 7"></polyline></svg>',
    chevronDown:
      '<svg class="stroked" viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"></polyline></svg>',
    hexagon:
    ...
    toggleLeft:
      '<svg class="stroked" viewBox="0 0 24 24"><rect x="1" y="5" width="22" height="14" rx="7" ry="7"></rect><circle cx="8" cy="12" r="3"></circle></svg>',
    ...
    chevronUp:
      '<svg class="stroked" viewBox="0 0 24 24"><polyline points="18 15 12 9 6 15"></polyline></svg>',
    arrowDownLeft:
      '<svg class="stroked" viewBox="0 0 24 24"><line x1="17" y1="7" x2="7" y2="17"></line><polyline points="17 17 7 17 7 7"></polyline></svg>',
    fileText:
    ...
    arrowLeftCircle:
      '<svg class="stroked" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"></circle><polyline points="12 8 8 12 12 16"></polyline><line x1="16" y1="12" x2="8" y2="12"></line></svg>',
    barChart2:
    ...
    thumbsDown:
      '<svg class="stroked" viewBox="0 0 24 24"><path d="M10 15v4a3 3 0 0 0 3 3l4-9V2H5.72a2 2 0 0 0-2 1.7l-1.38 9a2 2 0 0 0 2 2.3zm7-13h2.67A2.31 2.31 0 0 1 22 4v7a2.31 2.31 0 0 1-2.33 2H17"></path></svg>',
    ...
    rotateCcw:
      '<svg class="stroked" viewBox="0 0 24 24"><polyline points="1 4 1 10 7 10"></polyline><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"></path></svg>',
    hardDrive:
    ...
    cornerUpLeft:
      '<svg class="stroked" viewBox="0 0 24 24"><polyline points="9 14 4 9 9 4"></polyline><path d="M20 20v-7a4 4 0 0 0-4-4H4"></path></svg>',
    watch:
    ...
    refreshCcw:
      '<svg class="stroked" viewBox="0 0 24 24"><polyline points="1 4 1 10 7 10"></polyline><polyline points="23 20 23 14 17 14"></polyline><path d="M20.49 9A9 9 0 0 0 5.64 5.64L1 10m22 4l-4.64 4.36A9 9 0 0 1 3.51 15"></path></svg>',
    folderPlus:
    ...
    facebook:
      '<svg class="filled" version="1" viewBox="0 0 512 512"><path d="M464 0h-416c-26 0-48 22-48 48v416c0 26 22 48 48 48h208v-224h-64v-64h64v-32c0-53 43-96 96-96h64v64h-64c-18 0-32 14-32 32v32h96l-16 64h-80v224h144c26 0 48-22 48-48v-416c0-26-22-48-48-48z"></path></svg> ',
    codesandbox:
    ...
    cornerLeftDown:
      '<svg class="stroked" viewBox="0 0 24 24"><polyline points="14 15 9 20 4 15"></polyline><path d="M20 4h-7a4 4 0 0 0-4 4v12"></path></svg>',
    dollarSign:
    ...
    chevronsUp:
      '<svg class="stroked" viewBox="0 0 24 24"><polyline points="17 11 12 6 7 11"></polyline><polyline points="17 18 12 13 7 18"></polyline></svg>',
    uploadCloud:
    ...
    youtube:
      '<svg class="filled" version="1" viewBox="0 0 512 512"><g></g><path d="M507 154c0 0-5-35-20-51-20-20-41-21-51-22-72-5-179-5-179-5h-0c0 0-108 0-179 5-10 1-32 1-51 22-15 16-20 51-20 51s-5 41-5 83v39c0 41 5 83 5 83s5 35 20 51c20 20 45 20 57 22 41 4 174 5 174 5s108-0 179-5c10-1 32-1 51-22 15-16 20-51 20-51s5-41 5-83v-39c-0-41-5-83-5-83zM203 322v-144l138 72-138 72z"></path></svg> ',
    unlock:
    ...
    skipBack:
      '<svg class="stroked" viewBox="0 0 24 24"><polygon points="19 20 9 12 19 4 19 20"></polygon><line x1="5" y1="19" x2="5" y2="5"></line></svg>',
    filePlus:
    ...
    arrowUp:
      '<svg class="stroked" viewBox="0 0 24 24"><line x1="12" y1="19" x2="12" y2="5"></line><polyline points="5 12 12 5 19 12"></polyline></svg>',
    bellOff:
    ...
    chevronsLeft:
      '<svg class="stroked" viewBox="0 0 24 24"><polyline points="11 17 6 12 11 7"></polyline><polyline points="18 17 13 12 18 7"></polyline></svg>',
    calendar:
    ...
    arrowLeft:
      '<svg class="stroked" viewBox="0 0 24 24"><line x1="19" y1="12" x2="5" y2="12"></line><polyline points="12 19 5 12 12 5"></polyline></svg>',
    alignCenter:
    ...
    arrowDownRight:
      '<svg class="stroked" viewBox="0 0 24 24"><line x1="7" y1="7" x2="17" y2="17"></line><polyline points="17 7 17 17 7 17"></polyline></svg>',
    framer:
    ...
    arrowDownCircle:
      '<svg class="stroked" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"></circle><polyline points="8 12 12 16 16 12"></polyline><line x1="12" y1="8" x2="12" y2="16"></line></svg>',
    bookmark:
    ...
    cornerDownLeft:
      '<svg class="stroked" viewBox="0 0 24 24"><polyline points="9 10 4 15 9 20"></polyline><path d="M20 4v7a4 4 0 0 1-4 4H4"></path></svg>',
    monitor:
    ...
    chevronLeft:
      '<svg class="stroked" viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6"></polyline></svg>',
  ...
  var g0 = (i2, l4, s2, o2) => {
    ...
    for (let y2 of [
      ...
      if (l4 !== undefined) y2.setAttribute("fill", l4);
      if (s2 !== undefined) y2.setAttribute("stroke", s2);
      if (o2 !== undefined) y2.setAttribute("stroke-width", String(o2));
    }
    ...
    for (let y2 of [...h2]) {
      ...
      if (e2) y2.setAttribute("fill", X.fromCss(e2).html);
      if (x3) y2.setAttribute("stroke", X.fromCss(x3).html);
      if (n4) y2.setAttribute("strokeWidth", n4);
      if (p2) y2.setAttribute("strokeLinecap", p2);
      if (d2) y2.setAttribute("strokeLinejoin", d2);
  ...
  var t3 = new Proxy(O1, {
    get(i2, l4) {
      let s2 = O1[l4];
      if (l4 && !s2) console.warn(`icon ${l4} does not exist`);
      if (!s2) s2 = O1.square;
      return (...o2) => {
        let h2 = I.div();
        h2.innerHTML = s2;
        let r2 = h2.querySelector("svg"),
          y2 = new Set(r2.classList);
        y2.add("tosi-icon");
        let e2 = ZM.svg(
          {
            class: Array.from(y2).join(" "),
            viewBox: r2.getAttribute("viewBox"),
          },
          ...o2,
          ...r2.children
        );
        if (
          ((e2.style.strokeWidth = kE.tosiIconStrokeWidth("2px")),
          y2.has("filled"))
        )
          (e2.style.stroke = "none"), (e2.style.fill = "currentColor");
        else if (y2.has("stroked"))
          (e2.style.stroke = kE.tosiIconStroke("currentColor")),
            (e2.style.fill = "none");
        else
          (e2.style.stroke = kE.tosiIconStroke("currentColor")),
            (e2.style.fill = kE.tosiIconFill("currentColor"));
  ...
  class k0 extends u {
    ...
    render() {
      ...
      if (this.fill) i2.fill = this.fill;
      this.append(t3[this.icon]({ style: i2 }));
    }
  }
  var t32 = k0.elementCreator({
    tag: "tosi-icon",
    styleSpec: {
      ":host": {
      ...
      ":host, :host svg": { height: kE.tosiIconSize("16px") },
    },
  });
  var w0 = () => {};
  ...
  class v0 extends u {
    ...
    static styleSpec = {
      ...
      ":host .babylonVRicon:hover": { transform: "scale(1.1)" },
    };
    content = I.canvas({ part: "canvas" });
    constructor() {
      super();
      this.babylonReady = (async () => {
        let { BABYLON: i2 } = await Z(
    ...
    loadUI = async (i2) => {
      let { BABYLON: l4 } = await Z(
          "https://cdn.babylonjs.com/gui/babylon.gui.min.js",
          "BABYLON"
        ),
        s2 = l4.GUI.AdvancedDynamicTexture.CreateFullscreenUI(
          "GUI",
          true,
          this.scene
        ),
        { snippetId: o2, jsonUrl: h2, data: r2, size: y2 } = i2;
      if (y2) (s2.idealWidth = y2), (s2.renderAtIdealSize = true);
      let e2;
      if (o2) e2 = await s2.parseFromSnippetAsync(o2);
      else if (h2) e2 = await s2.parseFromURLAsync(h2);
      else if (r2) e2 = s2.parseContent(r2);
      else return null;
      let x3 = s2.getChildren()[0],
        n4 = x3.children.reduce((p2, d2) => {
    ...
    connectedCallback() {
      ...
      this.babylonReady.then(async (l4) => {
        if (
          ((this.BABYLON = l4),
          (this.engine = new l4.Engine(i2, true)),
          (this.scene = new l4.Scene(this.engine)),
          this.sceneCreated)
        )
          await this.sceneCreated(this, l4);
        if (this.scene.activeCamera === undefined)
          new l4.ArcRotateCamera(
            "default-camera",
            -Math.PI / 2,
            Math.PI / 2.5,
            3,
            new l4.Vector3(0, 0, 0)
          ).attachControl(this.parts.canvas, true);
  ...
  class P1 extends u {
    ...
    load = ({ bodymovin: i2 }) => {
      ...
      if (this.animation) {
        ...
        if (l4 !== null) l4.querySelector("svg")?.remove();
      }
      (this.animation = i2.loadAnimation(this.config)),
        this.animation.addEventListener("DOMLoaded", this.doneLoading);
    };
    render() {
      super.render(),
        P1.bodymovinAvailable.then(this.load).catch((i2) => {
          console.error(i2);
        });
    }
  }
  ...
  class d2 extends u {
    ...
    set page(i2) {
      ...
      if (this.lastPage <= 0) (o2.disabled = s2.disabled = true), (i2 = 0);
      else
        (i2 = Math.max(0, Math.min(this.lastPage, i2))),
          (i2 = isNaN(i2) ? 0 : i2);
      if (this._page !== i2)
        (this._page = isNaN(i2) ? 0 : i2),
          this.animateScroll(this._page * l4.offsetWidth),
          (s2.disabled = this.page <= 0 && !this.loop),
          (o2.disabled = this.page >= this.lastPage && !this.loop);
    }
    get visibleItems() {
      return [...this.children].filter(
        (i2) => getComputedStyle(i2).display !== "none"
      );
    }
    get lastPage() {
      return Math.max(
    ...
    static styleSpec = {
      ...
      ":host svg": { height: fM.carouselIconSize },
      ":host button": {
        outline: "none",
        border: "none",
        boxShadow: "none",
      ...
      ":host::part(back)": { left: 0 },
      ":host::part(forward)": { right: 0 },
      ":host button:disabled": { opacity: 0.5, pointerEvents: "none" },
      ":host button:hover": { color: fM.carouselButtonHoverColor },
      ":host button:active": { color: fM.carouselButtonActiveColor },
      ":host::part(pager)": { position: "relative" },
      ":host::part(scroller)": {
        overflow: "auto hidden",
        position: "relative",
      },
      ":host::part(grid)": { display: "grid", justifyItems: "center" },
      ...
      ":host .dot.current": { background: fM.carouselDotCurrentColor },
      ":host::part(progress)": {
        display: "flex",
    ...
    indicateCurrent = () => {
      let { scroller: i2, progress: l4 } = this.parts,
        s2 = i2.scrollLeft / i2.offsetWidth;
      [...l4.children].forEach((o2, h2) => {
        o2.classList.toggle(
          "current",
          Math.floor(h2 / this.maxVisibleItems - s2) === 0
        );
      }),
    ...
    snapPosition = () => {
      let { scroller: i2 } = this.parts,
        l4 = Math.round(i2.scrollLeft / i2.offsetWidth);
      if (l4 !== this.page)
    ...
    handleDotClick = (i2) => {
      let { progress: l4 } = this.parts,
        s2 = [...l4.children].indexOf(i2.target);
      if (s2 > -1) this.page = Math.floor(s2 / this.maxVisibleItems);
    };
    snapTimer;
    animationFrame;
    animateScroll(i2, l4 = -1, s2 = 0) {
      ...
      if (h2 >= this.snapDuration || Math.abs(o2.scrollLeft - i2) < 2)
        ...
        (o2.scrollLeft = l4 + this.easing(h2 / this.snapDuration) * (i2 - l4)),
          (this.animationFrame = requestAnimationFrame(() => {
            this.animateScroll(i2, l4, s2);
          }));
    }
    content = () => [
      E1(
        { part: "pager" },
        p2({ title: "previous slide", part: "back" }, t3.chevronLeft()),
        E1(
          { title: "slides", role: "group", part: "scroller" },
          E1({ part: "grid" }, m4())
        ),
        p2({ title: "next slide", part: "forward" }, t3.chevronRight())
      ),
      E1({ title: "choose slide to display", role: "group", part: "progress" }),
    ];
    connectedCallback() {
      super.connectedCallback(),
      ...
      i2.addEventListener("click", this.back),
        l4.addEventListener("click", this.forward),
        s2.addEventListener("scroll", this.indicateCurrent),
        o2.addEventListener("click", this.handleDotClick),
        (this.lastAutoAdvance = Date.now()),
    ...
    render() {
      super.render();
      let { dots: i2, arrows: l4, visibleItems: s2, lastPage: o2 } = this,
        { progress: h2, back: r2, forward: y2, grid: e2 } = this.parts;
      s2.forEach((x3) => {
        x3.role = "group";
      }),
        (e2.style.gridTemplateColumns = `${
          100 / this.maxVisibleItems / (1 + this.lastPage)
        }% `
          .repeat(s2.length)
          .trim()),
        (e2.style.width = (1 + this.lastPage) * 100 + "%"),
        (h2.textContent = ""),
        h2.append(
          ...s2.map((x3, n4) => p2({ title: `item ${n4 + 1}`, class: "dot" }))
        ),
        this.indicateCurrent(),
        (h2.style.display = i2 && o2 > 0 ? "" : "none"),
        (r2.hidden = y2.hidden = !(l4 && o2 > 0));
    }
  }
  var A3 = d2;
  var S4 = d2.elementCreator({
    tag: "tosi-carousel",
    styleSpec: {
      ":host": {
      ...
      ":host:focus": { outline: "none", boxShadow: "none" },
    },
  });
  var F3 = S4;
  ...
  class M1 extends u {
    ...
    set value(i2) {
      if (this.editor === undefined) this.source = i2;
      else
        this.editor.setValue(i2),
    ...
    connectedCallback() {
      ...
      if (this._editorPromise === undefined)
        (this._editorPromise = l5(this, this.mode, this.options, this.theme)),
          this._editorPromise.then(({ ace: i2, editor: l4 }) => {
            (this._ace = i2),
              (this._editor = l4),
              l4.setValue(this.source, 1),
              l4.clearSelection(),
              l4.session.getUndoManager().reset();
          });
    }
    render() {
      if ((super.render(), this._editorPromise !== undefined))
        this._editorPromise.then(({ editor: i2 }) =>
          i2.setReadOnly(this.disabled)
        );
    }
  }
  ...
  class H0 extends u {
    ...
    static styleSpec = {
      ...
      ":host::part(alpha)": { width: fM.alphaWidth },
      ":host::part(css)": { width: fM.cssWidth, fontFamily: "monospace" },
    };
    content = [
      f2({ title: "base color", type: "color", part: "rgb" }),
      f2({
        type: "range",
        title: "opacity",
        part: "alpha",
      ...
      f2({ title: "css color spec", part: "css" }),
    ];
    valueChanged = false;
    update = (i2) => {
      let { rgb: l4, alpha: s2, css: o2 } = this.parts;
      if (i2.type === "input")
        (this.color = X.fromCss(l4.value)),
          (this.color.a = Number(s2.value)),
          (o2.value = this.color.html);
      else
        (this.color = X.fromCss(o2.value)),
          (l4.value = this.color.html.substring(0, 7)),
          (s2.value = String(this.color.a));
      (l4.style.opacity = String(this.color.a)),
    ...
    connectedCallback() {
      ...
      i2.addEventListener("input", this.update),
        l4.addEventListener("input", this.update),
        s2.addEventListener("change", this.update);
    }
    render() {
      ...
      (this.color = X.fromCss(this.value)),
        (i2.value = this.color.html.substring(0, 7)),
        (i2.style.opacity = String(this.color.a)),
        (l4.value = String(this.color.a)),
        (s2.value = this.color.html);
    }
  }
  ...
  var G2 = (i2, l4, s2 = "move") => {
    if (!i2.type.startsWith("touch")) {
      ...
      let y2 = (e2) => {
        let x3 = e2.clientX - h2,
          n4 = e2.clientY - r2;
        if (l4(x3, n4, e2) === true)
          h1.removeEventListener("mousemove", y2),
            h1.removeEventListener("mouseup", y2),
            h1.remove();
      };
      h1.addEventListener("mousemove", y2, u1),
        h1.addEventListener("mouseup", y2, u1);
    } else if (i2 instanceof TouchEvent) {
      let h2 = i2.changedTouches[0],
        r2 = h2.identifier,
        y2 = h2.clientX,
        e2 = h2.clientY,
        x3 = i2.target,
        ...
        d3 = (C3) => {
          ...
          if (k3 !== undefined) (n4 = k3.clientX - y2), (p3 = k3.clientY - e2);
          if (C3.type === "touchmove")
            C3.stopPropagation(), C3.preventDefault();
          if (l4(n4, p3, C3) === true || k3 === undefined)
            x3.removeEventListener("touchmove", d3),
              x3.removeEventListener("touchend", d3),
              x3.removeEventListener("touchcancel", d3);
        };
      x3.addEventListener("touchmove", d3),
        x3.addEventListener("touchend", d3, u1),
        x3.addEventListener("touchcancel", d3, u1);
    }
  };
  var B1 = (i2 = "body *") =>
    [...document.querySelectorAll(i2)]
      .map((l4) => parseFloat(getComputedStyle(l4).zIndex))
      .reduce((l4, s2) => (isNaN(l4) || Number(l4) < s2 ? s2 : Number(l4)), 0);
  var t1 = (i2, l4 = "body *") => {
  ...
  class r1 extends u {
    ...
    reposition = (i2) => {
      if (i2.target?.closest(".no-drag")) return;
      if (this.drag) {
        t1(this);
        let s2 = this.offsetLeft,
          o2 = this.offsetTop;
        G2(i2, (h2, r2, y2) => {
          if (
            ((this.style.left = `${s2 + h2}px`),
            (this.style.top = `${o2 + r2}px`),
    ...
    connectedCallback() {
      ...
      let i2 = { passive: true };
      this.addEventListener("touchstart", this.reposition, i2),
        this.addEventListener("mousedown", this.reposition, i2),
        t1(this);
    }
    disconnectedCallback() {
  ...
  window.addEventListener(
    ...
    () => {
      Array.from(r1.floats).forEach((i2) => {
        if (i2.remainOnResize === "hide") i2.hidden = true;
        else if (i2.remainOnResize === "remove") i2.remove();
      });
    },
    { passive: true }
  );
  document.addEventListener(
    ...
    (i2) => {
      ...
      Array.from(r1.floats).forEach((l4) => {
        if (l4.remainOnScroll === "hide") l4.hidden = true;
        else if (l4.remainOnScroll === "remove") l4.remove();
      });
    },
  ...
  var A0 = (i2) => {
    let {
      ...
      e2 = Array.isArray(l4) ? m1(...l4) : m1(l4);
  ...
  var x5 = (i2, l4, s2, o2, h2, r2 = false) => {
    {
      ...
      if (w2 !== "fixed") i2.style.position = "fixed";
      if (h2) i2.remainOnResize = h2;
      if (o2) i2.remainOnScroll = o2;
      t1(i2);
    }
    i2.drag = r2;
    let {
        ...
        width: x3,
        height: n4,
      } = l4.getBoundingClientRect(),
      p3 = y2 + x3 * 0.5,
      d3 = e2 + n4 * 0.5,
      C3 = window.innerWidth,
      k3 = window.innerHeight;
    if (s2 === "side")
    ...
    if (
      ...
      (i2.style.bottom = (k3 - e2).toFixed(2) + "px"),
        (i2.style.left = p3.toFixed(2) + "px"),
        (i2.style.transform = "translateX(-50%)");
    else if (s2 === "s")
      (i2.style.top = (e2 + n4).toFixed(2) + "px"),
        (i2.style.left = p3.toFixed(2) + "px"),
        (i2.style.transform = "translateX(-50%)");
      ...
      (i2.style.left = (y2 + x3).toFixed(2) + "px"),
        (i2.style.top = d3.toFixed(2) + "px"),
        (i2.style.transform = "translateY(-50%)");
    else if (s2 === "w")
      (i2.style.right = (C3 - y2).toFixed(2) + "px"),
        (i2.style.top = d3.toFixed(2) + "px"),
        (i2.style.transform = "translateY(-50%)");
    i2.style.setProperty(
      "--max-height",
      `calc(100vh - ${i2.style.top || i2.style.bottom})`
    ),
      i2.style.setProperty(
        "--max-width",
        `calc(100vw - ${i2.style.left || i2.style.right})`
      );
  };
  function C22(i2, l4 = true) {
    return (s2, o2) => {
      ...
      for (let y2 in h2)
        if (h2[y2] !== r2[y2])
          return (Array.isArray(l4) ? l4[y2] !== false : l4)
            ? h2[y2] > r2[y2]
              ? 1
              : -1
            : h2[y2] > r2[y2]
  ...
  var q0 = (i2, l4) => {
    return !!i2.find((s2) => {
      if (s2 === null || l4 == null) return false;
      else if (Array.isArray(s2)) return q0(s2, l4);
      else if (s2.value === l4 || s2 === l4) return true;
    });
  };
  ...
  class T3 extends u {
    ...
    set options(i2) {
      if (typeof i2 === "string") this._options = T3.parseOptionsString(i2);
      else this._options = i2;
      this.queueRender();
    }
    static parseOptionsString(i2) {
      return i2.split(",").map((l4) => {
        ...
        if (!h2) return { value: o2, caption: o2 };
    ...
    setValue = (i2, l4 = false) => {
      if (this.value !== i2) (this.value = i2), this.queueRender(true);
      if (l4) this.dispatchEvent(new Event("action"));
    };
    getValue = () => this.value;
    get selectOptions() {
      return this.options;
    ...
    buildOptionMenuItem = (i2) => {
      ...
      let { setValue: l4, getValue: s2 } = this,
        o2,
        h2,
        r2;
      if (typeof i2 === "string") h2 = r2 = i2;
      else ({ icon: o2, caption: h2, value: r2 } = i2);
      if (this.localized) h2 = z2(h2);
      let { options: y2 } = i2;
      if (y2)
        return {
          icon: o2,
          caption: h2,
          checked: () => q0(y2, s2()),
          menuItems: y2.map(this.buildOptionMenuItem),
        };
      return {
        icon: o2,
        caption: h2,
        ...
        action:
          typeof r2 === "function"
            ? async () => {
                let e2 = await r2();
                if (e2 !== undefined) l4(e2, true);
              }
            : () => {
                if (typeof r2 === "string") l4(r2, true);
              },
      };
    };
    poppedOptions = [];
    get optionsMenu() {
      ...
      if (this.filter === "") return i2;
      let l4 = (s2) => {
        if (s2 === null) return true;
        else if (s2.menuItems)
          return (
            (s2.menuItems = s2.menuItems.filter(l4)), s2.menuItems.length > 0
          );
        else return s2.caption.toLocaleLowerCase().includes(this.filter);
      };
      return i2.filter(l4);
    }
    handleChange = (i2) => {
      let { value: l4 } = this.parts,
        s2 = l4.value || "";
    ...
    handleKey = (i2) => {
      if (i2.key === "Enter") i2.preventDefault();
    };
    filterMenu = FE(() => {
    ...
    popOptions = (i2) => {
      if (i2 && i2.type === "click") this.filter = "";
    ...
    content = () => [
      d5(
        { type: "button", part: "button", onClick: this.popOptions },
        F0(),
        f5({
          part: "value",
          value: this.value,
    ...
    get allOptions() {
      ...
      function l4(s2) {
        for (let o2 of s2)
          if (typeof o2 === "string") i2.push({ caption: o2, value: o2 });
          else if (o2?.value) i2.push(o2);
          else if (o2?.options) l4(o2.options);
      }
      return l4(this.selectOptions), i2;
    }
    findOption() {
      return (
        this.allOptions.find((l4) => l4.value === this.value) || {
          caption: this.value,
          value: this.value,
        }
      );
    ...
    connectedCallback() {
      ...
      if (this.localized) a2.allInstances.add(this);
    }
    disconnectedCallback() {
    ...
    render() {
      ...
      let s2 = i2.previousElementSibling,
        o2 = this.findOption(),
        h2 = F0();
      if (((i2.value = this.localized ? z2(o2.caption) : o2.caption), o2.icon))
        if (o2.icon instanceof HTMLElement) h2 = o2.icon.cloneNode(true);
        else h2 = t3[o2.icon]();
      s2.replaceWith(h2),
        i2.setAttribute(
          "placeholder",
          this.localized ? z2(this.placeholder) : this.placeholder
        ),
        (i2.style.pointerEvents = this.editable ? "" : "none"),
        (i2.readOnly = !this.editable);
    }
  }
  var h9 = T3;
  var X3 = T3.elementCreator({
    tag: "tosi-select",
    styleSpec: {
      ":host": {
      ...
      ":host:not([show-icon]) button > :first-child": { display: "none" },
      ":host[hide-caption] button > :nth-child(2)": { display: "none" },
      ':host [part="value"]': {
  ...
  TE.localeOptions = {
    toDOM(i2, l4) {
      if (i2 instanceof T3) i2.options = l4;
    },
  };
  var c9 = (i2) => {
    if (A.locales.value.includes(i2)) A.locale.value = i2;
    else console.error(`language ${i2} is not available`);
  };
  var O0 = () => {
    let i2 = Array.from(i1.allInstances);
    for (let l4 of i2) l4.localeChanged();
  };
  A.locale.observe(O0);
...
`
    ...
    if (l4 && s2 && o2 && h2) {
      if (
        ((A.locales.value = l4),
        (A.languages.value = s2),
        (A.emoji.value = o2),
        (A.stringMap.value = h2.reduce((r2, y2) => {
          return (r2[y2[0].toLocaleLowerCase()] = y2), r2;
        }, {})),
        (A.localeOptions.value = l4
          .map((r2, y2) => ({
            icon: _0({ title: l4[y2] }, o2[y2]),
            caption: s2[y2],
            value: r2,
          }))
          .sort(g5)),
        !A.locales.value.includes(A.locale.value))
      ) {
        let r2 = A.locale.value.substring(0, 2);
        A.locale.value =
          A.locales.value.find((y2) => y2.substring(0, 2) === r2) ||
          A.locales.value[0];
      }
      O0();
    }
  ...
  function z2(i2) {
    if (i2.endsWith("…")) return z2(i2.substring(0, i2.length - 1)) + "…";
    let l4 = A.locales.value.indexOf(A.locale.value);
    if (l4 > -1) {
      let o2 = A.stringMap.value[i2.toLocaleLowerCase()],
        h2 = o2 && o2[l4];
      if (h2) i2 = i2.toLocaleLowerCase() === i2 ? h2.toLocaleLowerCase() : h2;
    }
    return i2;
  }
  ...
  class g2 extends u {
    ...
    content = () => {
      return X3({
        ...
        title: z2("Language"),
        bindValue: A.locale,
        bindLocaleOptions: A.localeOptions,
      });
    };
    render() {
      super.render(),
  ...
  class i1 extends u {
    ...
    localeChanged() {
      if (!this.refString) this.refString = this.textContent || "";
  ...
  var a2 = i1;
  var l1 = i1.elementCreator({
    tag: "tosi-localized",
    styleSpec: { ":host": { pointerEvents: "none" } },
  });
  var C9 = l1;
  var P0 = (i2, l4) => {
    l4 = l4.toLocaleLowerCase();
    let s2 = !!l4.match(/\^|ctrl/),
      o2 = !!l4.match(/⌘|meta/),
      h2 = !!l4.match(/⌥|⎇|alt|option/),
      r2 = !!l4.match(/⇧|shift/),
      y2 = l4.slice(-1);
    return (
      i2.key === y2 &&
  ...
  hL("xin-menu-helper", {
    ".xin-menu": {
      ...
      maxHeight: `calc(${fM.maxHeight} - ${kE.menuInset("8px")})`,
      borderRadius: fM.spacing50,
      background: kE.menuBg("#fafafa"),
      boxShadow: kE.menuShadow(
        `${fM.spacing13} ${fM.spacing50} ${fM.spacing} #0004`
      ),
    },
    ".xin-menu > div": { width: kE.menuWidth("auto") },
    ".xin-menu-trigger": {
      paddingLeft: 0,
      paddingRight: 0,
      minWidth: kE.touchSize("48px"),
    },
    ".xin-menu-separator": {
      display: "inline-block",
      content: " ",
    ...
    ".xin-menu-item, .xin-menu-item > span": {
      color: kE.menuItemColor("#222"),
    },
    ".xin-menu-with-icons .xin-menu-item": {
      gridTemplateColumns: "30px 1fr 30px",
    },
    ".xin-menu-item svg": { stroke: kE.menuItemIconColor("#222") },
    ".xin-menu-item.xin-menu-item-checked": {
      background: kE.menuItemHoverBg("#eee"),
    },
    ...
    ".xin-menu-item:active": {
      boxShadow: "none !important",
      background: kE.menuItemActiveBg("#aaa"),
      color: kE.menuItemActiveColor("#000"),
    },
    ".xin-menu-item:active svg": { stroke: kE.menuItemIconActiveColor("#000") },
  });
  var H5 = (i2, l4) => {
    let s2 = (i2.checked && i2.checked() && "check") || false,
      o2 = i2?.icon || s2 || K2(" ");
    if (typeof o2 === "string") o2 = t3[o2]();
    let h2 = l4.role === "listbox" ? "option" : "menuitem",
      r2;
    if (typeof i2?.action === "string")
      r2 = B5(
        { class: "xin-menu-item", role: h2, href: i2.action },
        o2,
        l4.localized ? K2(z2(i2.caption)) : K2(i2.caption),
        K2(i2.shortcut || " ")
      );
    else
      r2 = k22(
        { class: "xin-menu-item", role: h2, onClick: i2.action },
        o2,
        l4.localized ? K2(z2(i2.caption)) : K2(i2.caption),
        K2(i2.shortcut || " ")
      );
    if (
      (r2.classList.toggle("xin-menu-item-checked", s2 !== false),
      l4.role === "listbox" && s2)
    )
  ...
  var j5 = (i2, l4) => {
    ...
    let h2 = k22(
      {
        class: "xin-menu-item",
        disabled: !(!i2.enabled || i2.enabled()),
        onClick(r2) {
          J2(
            Object.assign({}, l4, {
              menuItems: i2.menuItems,
              target: h2,
              submenuDepth: (l4.submenuDepth || 0) + 1,
              position: "side",
            })
          ),
            r2.stopPropagation(),
      ...
      o2,
      l4.localized ? K2(z2(i2.caption)) : K2(i2.caption),
      t3.chevronRight({ style: { justifySelf: "flex-end" } })
    );
    return h2;
  };
  var A5 = (i2, l4) => {
    if (i2 === null) return K2({ class: "xin-menu-separator" });
    else {
  ...
  var F5 = (i2) => {
    let { target: l4, width: s2, menuItems: o2, role: h2 = "menu" } = i2,
      r2 = o2.find((y2) => y2?.icon || y2?.checked);
    return $0(
      {
        class: r2 ? "xin-menu xin-menu-with-icons" : "xin-menu",
        role: h2,
        onClick() {
          p1(0);
        },
      ...
      $0(
        {
          style: {
            minWidth: l4.offsetWidth + "px",
            width: typeof s2 === "number" ? `${s2}px` : s2,
          },
          onMousedown(y2) {
  ...
  var p1 = (i2 = 0) => {
    let l4 = d1.splice(i2);
    for (let s2 of l4) if ((s2.menu.remove(), s2.onClose)) s2.onClose();
    return (z1 = l4[0]), i2 > 0 ? d1[i2 - 1] : undefined;
  };
  document.body.addEventListener("mousedown", (i2) => {
    if (i2.target && !d1.find((l4) => l4.target.contains(i2.target))) p1(0);
  });
  document.body.addEventListener("keydown", (i2) => {
    if (i2.key === "Escape") p1(0);
  });
  var J2 = (i2) => {
    i2 = Object.assign({ submenuDepth: 0 }, i2);
    let { target: l4, position: s2, submenuDepth: o2 } = i2;
    if (z1 && !document.body.contains(z1?.menu)) z1 = undefined;
    if (d1.length && !document.body.contains(d1[0].menu)) d1.splice(0);
    if (o2 === 0 && z1?.target === l4) return;
    let h2 = p1(o2);
    if (z1?.target === l4) return;
    if (h2 && h2.target === l4) {
      p1();
      return;
    }
    if (!i2.menuItems?.length) return;
    let r2 = F5(i2),
      y2 = A0({ content: r2, target: l4, position: s2 });
    (y2.remainOnScroll = "remove"),
      d1.push({ target: l4, menu: y2, onClose: i2.onClose });
  };
  function J0(i2, l4) {
    for (let s2 of i2) {
      if (!s2) continue;
  ...
  class w2 extends u {
    ...
    content = () =>
      k22({ tabindex: 0, part: "trigger", onClick: this.showMenu }, z5());
    handleShortcut = async (i2) => {
      let l4 = J0(this.menuItems, i2);
      if (l4) {
        if (l4.action instanceof Function) l4.action();
      }
    };
    constructor() {
      super();
  ...
  var G0 = (i2, l4) => {
    if (!i2) return false;
    for (let s2 of i2)
      if (s2 === "special/any") return true;
      else if (s2.indexOf("*") > -1) {
        let [o2, h2] = s2.split("/"),
          [r2, y2] = l4.split("/");
        if ((o2 === "*" || o2 === r2) && (h2 === "*" || h2 === y2)) return true;
      } else if (s2 === l4) return true;
  };
  var S1 = (i2) => {
  ...
  var Z0 = (i2) => {
    ...
    for (let s2 of l4) {
      ...
      if (i2.find((h2) => G0(o2, h2))) s2.classList.add("drag-target");
      else s2.classList.remove("drag-target");
    }
  };
  function L5(i2) {
    ...
    let s2 = l4.matches('[draggable="true"]')
      ? v22(l4.dataset.drag || "text/html")
      : v22(l4.dataset.drag || "url");
    for (let o2 of s2) {
      let h2 =
        l4.dataset.dragContent ||
        (o2 === "text/html" ? l4.innerHTML : l4.textContent);
      i2.dataTransfer?.setData(o2, h2 || "");
    }
    Z0(i2.dataTransfer?.types), i2.stopPropagation();
  }
  function Q0(i2) {
    if (!V5()) Z0(i2.dataTransfer?.types);
    let l4 = i2.target.closest(".drag-target");
    if (l4 && i2.dataTransfer)
      l4.classList.add("drag-over"), (i2.dataTransfer.dropEffect = "copy");
    else i2.preventDefault(), i2.stopPropagation();
  }
  function _5() {
    S1("drag-over");
  }
    ...
    if (l4) {
      ...
      for (let o2 of s2)
        if (G0(i2.dataTransfer?.types, o2))
          ...
          else l4.textContent = i2.dataTransfer?.getData(o2) || "";
    }
    U0();
  }
  ...
  var M2 = () => {
    if (b0) return;
    document.body.addEventListener("dragstart", L5),
      document.body.addEventListener("dragenter", Q0),
      document.body.addEventListener("dragover", Q0),
      document.body.addEventListener("drop", O5),
      document.body.addEventListener("dragleave", _5),
      document.body.addEventListener("dragend", U0),
      window.addEventListener("dragover", (i2) => i2.preventDefault()),
  ...
  function b5(i2, l4, s2) {
    ...
    if (o2 !== undefined) {
      ...
      switch (typeof h2) {
        case "string":
          if (h2.match(/^\d+(\.\d+)?$/)) return 6 * s2;
          else if (h2.includes(" ")) return 20 * s2;
          else return 12 * s2;
        case "number":
          return 6 * s2;
        case "boolean":
  ...
  class H2 extends u {
    ...
    set filter(i2) {
      if (this._filter !== i2) (this._filter = i2), this.queueRender();
    }
    get sort() {
      if (this._sort) return this._sort;
      let i2 = this._columns?.find(
        (s2) => s2.sort === "ascending" || s2.sort === "descending"
      );
      if (!i2) return;
      let { prop: l4 } = i2;
      return i2.sort === "ascending"
        ? (s2, o2) => (s2[l4] > o2[l4] ? 1 : -1)
        : (s2, o2) => (s2[l4] > o2[l4] ? -1 : 1);
    }
    set sort(i2) {
      if (this._sort !== i2) (this._sort = i2), this.queueRender();
    }
    get columns() {
      if (!Array.isArray(this._columns)) {
        ...
        this._columns = Object.keys(i2[0] || {}).map((l4) => {
          ...
          return {
            name: l4.replace(/([a-z])([A-Z])/g, "$1 $2").toLocaleLowerCase(),
            prop: l4,
            align:
              typeof i2[0][l4] === "number" ||
              (i2[0][l4] !== "" && !isNaN(i2[0][l4]))
                ? "right"
                : "left",
            visible: s2 !== false,
            width: s2 ? s2 : 0,
    ...
    getColumn(i2) {
      let l4 =
          (i2.touches !== undefined ? i2.touches[0].clientX : i2.clientX) -
          this.getBoundingClientRect().x,
        s2 = i2.touches !== undefined ? 20 : 5,
        o2 = 0,
        h2 = [];
    ...
    setCursor = (i2) => {
      if (this.getColumn(i2) !== undefined) this.style.cursor = "col-resize";
      else this.style.cursor = "";
    };
    resizeColumn = (i2) => {
      let l4 = this.getColumn(i2);
      if (l4 !== undefined) {
        let s2 = Number(l4.width),
          o2 = i2.touches !== undefined,
          h2 = o2 ? i2.touches[0].identifier : undefined;
        G2(
          i2,
          (r2, y2, e2) => {
            if (
              (o2
                ? [...e2.touches].find((p3) => p3.identifier === h2)
                : true) === undefined
            )
              return true;
            let n4 = s2 + r2;
            if (
              ((l4.width = n4 > this.minColumnWidth ? n4 : this.minColumnWidth),
    ...
    selectRow(i2, l4 = true) {
      if (l4) i2[this.selectedKey] = true;
      else delete i2[this.selectedKey];
    }
    selectRows(i2, l4 = true) {
      for (let s2 of i2 || this.array) this.selectRow(s2, l4);
    }
    deSelect(i2) {
    ...
    updateSelection = (i2) => {
      ...
      let { target: l4 } = i2;
      if (!(l4 instanceof HTMLElement)) return;
      ...
      if (
        ...
        let e2 =
          ...
          [x3, n4] = [
            this.rangeStart !== undefined ? y2.indexOf(this.rangeStart) : 0,
            y2.indexOf(o2),
          ].sort((p3, d3) => p3 - d3);
        if (x3 > -1)
    ...
    setColumnWidths() {
      let i2 = this.visibleColumns.map((s2) => s2.width + "px").join(" "),
        l4 = this.visibleColumns.reduce((s2, o2) => s2 + o2.width, 0) + "px";
      this.style.setProperty("--tosi-table-grid-columns", i2),
        this.style.setProperty("--tosi-table-grid-row-width", l4),
        this.style.setProperty("--grid-columns", i2),
        this.style.setProperty("--grid-row-width", l4);
    }
    ...
    popColumnMenu = (i2, l4) => {
      let { sortByColumn: s2 } = this,
        o2 = this.columns.filter((y2) => y2.visible === false),
        h2 = this.queueRender.bind(this),
        r2 = [];
      if (!this.nosort && l4.sort !== false)
        r2.push(
          {
            caption: this.localized
              ? `${z2("Sort")} ${z2("Ascending")}`
              : "Sort Ascending",
            icon: "sortAscending",
            action() {
              s2(l4);
    ...
    headerCell = (i2) => {
      ...
      let h2 = !(this.nosort && this.nohide)
        ? G5(
            {
              class: "menu-trigger",
              onClick(r2) {
        ...
        : {};
      return i2.headerCell !== undefined
        ? i2.headerCell(i2)
        : z22(
            {
              class: "th",
              role: "columnheader",
              ariaSort: s2,
              style: { ...this.cellStyle, justifyContent: i2.align || "left" },
            },
            this.captionSpan(
              { style: { flex: "1" } },
              typeof i2.name === "string" ? i2.name : i2.prop
            ),
            h2
          );
    };
    dataCell = (i2) => {
      if (i2.dataCell !== undefined) return i2.dataCell(i2);
      return z22({
        class: "td",
        role: "cell",
        style: { ...this.cellStyle, justifyContent: i2.align || "left" },
        ...
        H1(
          {
            class: "tr",
            role: "row",
            bind: { value: "^", binding: { toDOM: this.selectBinding } },
          },
          ...i2.map(this.dataCell)
        )
      );
    }
    draggedColumn;
    dropColumn = (i2) => {
      let l4 = i2.target.closest(".drag-over"),
        s2 = Array.from(l4.parentElement.children).indexOf(l4),
        o2 = this.visibleColumns[s2],
        h2 = this.columns.indexOf(this.draggedColumn),
        r2 = this.columns.indexOf(o2);
      this.columns.splice(h2, 1),
        this.columns.splice(r2, 0, this.draggedColumn),
        console.log({
          event: i2,
          target: l4,
          targetIndex: s2,
    ...
    render() {
      ...
      let { sort: i2 } = this;
      if (i2) this.rowData.visible.sort(i2);
      (this.textContent = ""),
          ...
          H1(
            {
              ...
              bindList: { value: this.rowData.visible, virtual: this.virtual },
            },
            this.rowTemplate(l4)
          )
        ),
  ...
  var Y0 = H2.elementCreator({
    ...
    styleSpec: {
      ...
      ":host .thead, :host .tbody": { width: fM.tosiTableGridRowWidth },
      ":host .tr": {
        display: "grid",
      ...
      ':host [draggable="true"]': { cursor: "ew-resize" },
    ...
    static async alert(i2, l4 = "Alert") {
      return new Promise((s2) => {
        let o2 = _2(
          ...
          i2.includes(`
`)
            ? I.pre({ style: { whiteSpace: "pre-wrap", margin: 0 } }, i2)
            : L2(i2)
        );
    ...
    content = () =>
      K5(
        ...
        Y5(q2({ name: "header" })),
        q2(),
        N5(q2({ name: "footer" }), F2({ part: "ok", onClick: this.ok }, "OK"))
      );
  }
  var _2 = W0.elementCreator({
    tag: "tosi-dialog",
    styleSpec: {
      ":host > dialog::backdrop": { backdropFilter: "blur(8px)" },
      ":host > dialog:not([open])": { display: "none" },
      ":host > dialog[open]": {
        minWidth: 300,
        border: 0,
        borderRadius: 10,
        ...
        flexDirection: "column",
        gap: 5,
        _dialogShadow: kE.menuShadow("0 5px 10px #0004"),
        _dialogBackground: kE.background("#fafafa"),
        _dialogColor: kE.textColor("#222"),
        boxShadow: fM.dialogShadow,
        background: fM.dialogBackground,
        color: fM.dialogColor,
      },
      ":host > dialog > *": { padding: "0 20px" },
      ":host > dialog > header": {
        display: "flex",
        justifyContent: "center",
        gap: 10,
  ...
  function E0(i2, l4) {
    if (l4 == null) l4 = "";
    else if (typeof l4 !== "string") l4 = String(l4);
...
`
          ...
          .reduce((s2, o2) => {
            if (o2.startsWith("<") || s2.length === 0) s2.push(o2);
            else {
...
` + o2;
        ...
        this.innerHTML = l4
          .map((s2) =>
            s2.startsWith("<") && s2.endsWith(">") ? s2 : k2(s2, this.options)
          )
          .join("");
      } else this.innerHTML = k2(i2, this.options);
      this.didRender();
    }
  }
  var E9 = O2;
  ...
  class P22 extends u {
    ...
    value = 0;
    makeTab(i2, l4, s2) {
      let o2 = l4.getAttribute("name"),
        h2 =
          l4.querySelector('template[role="tab"]')?.content.cloneNode(true) ||
          (this.localized ? l1(o2) : m5(o2));
      return f1(
        h2,
        { part: "tab", tabindex: 0, role: "tab", ariaControls: s2 },
        l4.hasAttribute("data-close")
          ? S5({ title: "close", class: "close" }, t3.x())
          : {}
      );
    }
    static styleSpec = {
      ...
      'slot[name="after-tabs"]': { flex: "0 0 auto" },
      ...
      ":host .elastic": { flex: "1" },
      ":host .border": { background: fM.tosiTabsBarColor },
      ...
      ":host button.close > svg": { height: "12px" },
    };
    onCloseTab = null;
    content = [
      f1(
        ...
        f1(
          { part: "tabrow" },
          f1({ class: "tabs", part: "tabs" }),
          f1({ class: "elastic" }),
          I0({ name: "after-tabs" })
        ),
        f1({ class: "border" }, f1({ class: "selected", part: "selected" }))
      ),
      I0(),
    ];
    addTabBody(i2, l4 = false) {
      if (!i2.hasAttribute("name"))
        throw (
          (console.error("element has no name attribute", i2),
          Error("element has no name attribute"))
        );
    ...
    keyTab = (i2) => {
      let { tabs: l4 } = this.parts,
        s2 = [...l4.children].indexOf(i2.target);
      switch (i2.key) {
        case "ArrowLeft":
          (this.value =
            (s2 + Number(l4.children.length) - 1) % l4.children.length),
            l4.children[this.value].focus(),
            i2.preventDefault();
          break;
        case "ArrowRight":
          (this.value = (s2 + 1) % l4.children.length),
            l4.children[this.value].focus(),
            i2.preventDefault();
          break;
        case " ":
    ...
    pickTab = (i2) => {
      let { tabs: l4 } = this.parts,
        s2 = i2.target,
        o2 = s2.closest("button.close") !== null,
        h2 = s2.closest(".tabs > div"),
        r2 = [...l4.children].indexOf(h2);
      if (o2) {
        let y2 = this.bodies[r2];
        if (!this.onCloseTab || this.onCloseTab(y2) !== false)
          this.removeTabBody(this.bodies[r2]);
      } else if (r2 > -1) this.value = r2;
    };
    setupTabs = () => {
      let { tabs: i2 } = this.parts,
        l4 = [...this.children].filter(
          (s2) => !s2.hasAttribute("slot") && s2.hasAttribute("name")
        );
      if (((i2.textContent = ""), this.value >= l4.length))
        this.value = l4.length - 1;
      for (let s2 in l4) {
        let o2 = l4[s2],
          h2 = `${this.instanceId}-${s2}`;
        o2.id = h2;
    ...
    connectedCallback() {
      super.connectedCallback();
      let { tabs: i2 } = this.parts;
      i2.addEventListener("click", this.pickTab),
        i2.addEventListener("keydown", this.keyTab),
        this.setupTabs(),
        i1.allInstances.add(this);
    }
    disconnectedCallback() {
      super.disconnectedCallback(), i1.allInstances.delete(this);
    }
    ...
    render() {
      let { tabs: i2, selected: l4 } = this.parts,
        s2 = this.bodies;
      for (let o2 = 0; o2 < s2.length; o2++) {
        ...
        if (this.value === Number(o2))
          r2.setAttribute("aria-selected", "true"),
            (l4.style.marginLeft = `${r2.offsetLeft - i2.offsetLeft}px`),
            (l4.style.width = `${r2.offsetWidth}px`),
            h2.toggleAttribute("hidden", false);
        else
          r2.toggleAttribute("aria-selected", false),
            h2.toggleAttribute("hidden", true);
      }
    }
  }
  var o6 = P22;
  ...
  function C1(i2, l4) {
    ...
    for (let o2 of l4)
      s2 = s2.replace(
        new RegExp(`import \\{(.*)\\} from '${o2}'`, "g"),
        `const {$1} = ${o2.replace(/-/g, "")}`
      );
    return s2;
  }
  async function ii(i2, l4, s2) {
    let o2 = C1(i2, Object.keys(l4)),
      h2 = s2(o2, { transforms: ["typescript"] }).code,
      r2 = Object.keys(l4).map((x3) => x3.replace(/-/g, "")),
      y2 = Object.values(l4);
  ...
  class i2 {
    ...
    handlePayload = (i3) => {
      if (i3.sentAt <= this.lastUpdate) return;
      if (i3.remoteKey !== this.remoteKey) return;
      (this.lastUpdate = i3.sentAt), this.onReceive(i3);
    };
    handleMessage = (i3) => {
      let l4 = i3.data;
      if (l4) this.handlePayload(l4);
    };
    handlePoll = () => {
      let i3 = null;
      try {
    ...
    for (let r2 of Object.values(l4))
      if (r2 && typeof r2 === "object") {
        for (let y2 of Object.values(r2))
          if (typeof y2 === "function" && "tagName" in y2) o2(y2.tagName);
      }
    let h2 = i3.document;
    if (h2) {
  ...
  async function b22(i3) {
    ...
    if (d3) d3.replaceWith(p3);
    else y2.insertBefore(p3, x3);
    let C3 = { preview: p3, ...h2 };
    try {
      let k3 = C1(o2, Object.keys(h2)),
        w3 = r2(k3, { transforms: ["typescript"] }).code,
        O = Object.keys(C3).map((F4) => F4.replace(/-/g, "")),
        R2 = Object.values(C3);
      await new U1(...O, w3)(...R2);
    } catch (k3) {
      if ((console.error(k3), n4)) n4(k3);
      else window.alert(`Error: ${k3}, the console may have more information…`);
    }
    return p3;
  }
  async function G22(i3) {
    let {
      ...
      n4 = y2.querySelector("iframe.preview-iframe");
    if (!n4) {
      (n4 = document.createElement("iframe")),
        (n4.className = "preview-iframe"),
        (n4.style.cssText = "width: 100%; height: 100%; border: none;");
      let w3 = y2.querySelector(".preview");
      if (w3) w3.replaceWith(n4);
      else y2.insertBefore(n4, e2);
    }
    let p3 = n4.contentDocument;
    if (!p3) return console.error("Could not access iframe document"), null;
    let d3 = n4.contentWindow;
    if (h2.tosijs) d3.tosijs = h2.tosijs;
    if (h2["tosijs-ui"]) d3.tosijsui = h2["tosijs-ui"];
    p3.open(),
      p3.write(`<!DOCTYPE html>
<html>
<head>
  <style>
    body { margin: 0; }
...
<body>
  <div class="preview">${l4}</div>
    ...
    try {
      let w3 = C1(o2, Object.keys(h2)),
        O = r2(w3, { transforms: ["typescript"] }).code,
        R2 = d3.eval("(async () => {}).constructor"),
        H = Object.keys(k3).map((v1) => v1.replace(/-/g, "")),
        F4 = Object.values(k3);
      await new R2(...H, O)(...F4);
    } catch (w3) {
      if ((console.error(w3), x3)) x3(w3);
      else window.alert(`Error: ${w3}, the console may have more information…`);
    }
    return C3;
  }
  function U2(i3, l4, s2, o2) {
    let h2 = [
      ...
      .filter((r2) => !r2.closest(o2))
      .map((r2) => ({
        block: r2.parentElement,
        language: r2.classList[0].split("-").pop(),
        code: r2.innerText,
      }));
    for (let r2 = 0; r2 < h2.length; r2 += 1) {
      ...
      while (
        r2 < h2.length - 1 &&
        h2[r2].block.nextElementSibling === h2[r2 + 1].block
      )
        y2.push(h2[r2 + 1]), (r2 += 1);
      let e2 = s2({ context: l4 });
      y2[0].block.parentElement.insertBefore(e2, y2[0].block),
        y2.forEach((n4) => {
          switch (n4.language) {
            case "js":
              e2.js = n4.code;
  ...
  var S0 = {
    ...
    ".-maximize": { zIndex: 101 },
    ":host.-vertical": { flexDirection: "column" },
    ":host .layout-indicator": {
      transition: "0.5s ease-out",
      transform: "rotateZ(270deg)",
    },
    ":host.-vertical .layout-indicator": { transform: "rotateZ(180deg)" },
    ...
    ':host [part="exampleWidgets"] svg': { stroke: "var(--widget-color)" },
    ":host .code-editors": {
      overflow: "hidden",
      background: "white",
      position: "relative",
    ...
    ":host .code-editors:not([hidden])": { display: "flex" },
    ...
    ':host.-test-passed [part="exampleWidgets"]': { "--widget-color": "#0a0" },
    ':host.-test-failed [part="exampleWidgets"]': { "--widget-color": "#f00" },
    ...
    ':host [part="testResults"][hidden]': { display: "none" },
    ":host .test-pass": { color: "#0a0" },
    ":host .test-fail": { color: "#f00" },
  };
  ...
  function T0(i3, l4) {
    if (i3 === l4) return true;
    if (typeof i3 !== typeof l4) return false;
    if (i3 === null || l4 === null) return i3 === l4;
    if (typeof i3 !== "object") return false;
    let s2 = i3,
      o2 = l4;
    if (Array.isArray(s2) !== Array.isArray(o2)) return false;
    let h2 = Object.keys(s2),
      r2 = Object.keys(o2);
    if (h2.length !== r2.length) return false;
    return h2.every((y2) => T0(s2[y2], o2[y2]));
  }
  var yi = 5000;
  var ei = {
    stringify(i3) {
      if (typeof i3 > "u") return "undefined";
      if (i3 === null) return "null";
      if (typeof Element < "u" && i3 instanceof Element)
        return `<${i3.tagName.toLowerCase()}>`;
      if (typeof Node < "u" && i3 instanceof Node) return `[${i3.nodeName}]`;
      try {
  ...
  function i4(i3, l4 = false) {
    let s2 = (h2, r2) => {
      if (!(l4 ? !h2 : h2)) throw new Z2(l4 ? `not: ${r2}` : r2);
    };
    return {
      toBe(h2) {
      ...
      toContain(h2) {
        ...
        else throw new Z2("toContain requires string or array");
      },
      toHaveLength(h2) {
        let r2 = i3.length;
  ...
  function ti(i3, l6, s2) {
    return Promise.race([
      i3,
      new Promise((o2, h2) =>
        setTimeout(() => h2(Error(`Test "${s2}" timed out after ${l6}ms`)), l6)
      ),
    ]);
  }
      ...
      test(h2, r2) {
        ...
        try {
          ...
          if (e2 instanceof Promise) {
            ...
            o2.push(x3);
          } else i3.push({ name: y2, passed: true });
        } catch (e2) {
  ...
  async function X22(i3, l6, s2, o2) {
    let h2 = [],
      ...
      y2 = {
        ...
        expect: r2.expect,
        test: r2.test,
        describe: r2.describe,
        waitMs: xi,
        waitFor: (e2, x3) => ni(l6, e2, x3),
      };
    try {
      let e2 = C1(i3, Object.keys(s2)),
        x3 = o2(e2, { transforms: ["typescript"] }).code,
        n4 = Object.keys(y2).map((C3) => C3.replace(/-/g, "")),
        p3 = Object.values(y2);
      await new U1(...n4, x3)(...p3);
    } catch (e2) {
      h2.push({ name: "Test execution", passed: false, error: e2.message });
    }
    if (r2.pending.length > 0) await Promise.all(r2.pending);
    return {
      passed: h2.filter((e2) => e2.passed).length,
      failed: h2.filter((e2) => !e2.passed).length,
      tests: h2,
    };
  }
  ...
  function gi() {
    if (typeof localStorage > "u") return false;
    let i3 = localStorage.getItem(K22);
    if (i3 !== null) return i3 === "true";
    return ai;
  }
  var { testManager: Y } = xE({ testManager: { enabled: gi() } });
  function l22() {
    document.body.classList.toggle("tests-enabled", Y.enabled.value),
  ...
  if (typeof document < "u")
    if (document.body) l22();
    else document.addEventListener("DOMContentLoaded", l22);
  function y4() {
    localStorage.setItem(K22, "true"),
      (Y.enabled.value = true),
      l22(),
      document.querySelectorAll("tosi-example").forEach((i3) => {
        i3.refresh();
      });
  }
  function e4() {
  ...
  class g1 extends u {
    ...
    getEditorValue(i3) {
      if (!this.hydrated) return this.pendingValues[i3] ?? "";
    ...
    updateUndo = () => {
      ...
      } else (l6.disabled = true), (s2.disabled = true);
      this.updateTestResultsVisibility();
    };
    updateTestResultsVisibility() {
      let { testResults: i3 } = this.parts,
        l6 = this.testResults,
        s2 = this.activeTab?.getAttribute("name") === "test",
        o2 = l6 && l6.failed > 0;
      i3.hidden = !l6 || l6.tests.length === 0 || (!s2 && !o2);
    }
    undo = () => {
      let { activeTab: i3 } = this;
      if (i3 instanceof M1) i3.editor.undo();
    };
    redo = () => {
      let { activeTab: i3 } = this;
      if (i3 instanceof M1) i3.editor.redo();
    };
    get isMaximized() {
    ...
    exampleMenu = () => {
      ...
      J2({
        ...
        width: "auto",
        menuItems: [
          { icon: "edit2", caption: "view/edit code", action: this.showCode },
          {
            icon: "edit",
      ...
      if (i3.metaKey || i3.ctrlKey) {
        ...
        switch (i3.key) {
          ...
          case "/":
            ...
            break;
          case "c":
            if (i3.shiftKey) this.copy(), (l6 = true);
            break;
        }
        if (l6) i3.preventDefault(), i3.stopPropagation();
      }
    };
    content = () => [
      j1(
        { part: "example" },
        Ci({ part: "style" }),
        j1({ part: "testIndicator", title: "test status" }),
        o4({ part: "testResults", hidden: true }),
        a1(
          {
            title: "example menu",
            part: "exampleWidgets",
      ...
      j1(
        ...
        T1(
          { part: "editors", onChange: this.updateUndo },
          $1({ name: "js", mode: "javascript", part: "js" }),
          $1({ name: "html", mode: "html", part: "html" }),
          $1({ name: "css", mode: "css", part: "css" }),
          $1({ name: "test", mode: "javascript", part: "test" }),
          j1(
            ...
            a1(
              ...
              t3.columns({ class: "layout-indicator" })
            ),
            a1(
              {
      ...
      fi({ part: "sources", hidden: true }),
    ];
    connectedCallback() {
      super.connectedCallback(), this.flushPendingValues();
      let { sources: i3 } = this.parts;
      this.initFromElements([...i3.children]),
        (this.remoteSync = new i2(this.storageKey, this.remoteKey, (s2) => {
          if (s2.close) {
    ...
    closeCode = () => {
      if (this.remoteId !== "") this.remoteSync?.sendClose(), window.close();
      else
        this.remoteSync?.sendClose(),
          this.classList.remove("-maximize"),
          (this.parts.codeEditors.hidden = true);
    ...
    openEditorWindow = () => {
      ...
      m0(this.prefix, this.uuid, this.storageKey, this.remoteKey, {
        css: i3,
        html: l6,
        js: s2,
        test: o2,
    ...
    refresh = async () => {
      ...
      let i3 = await $2(),
        { example: l6, style: s2, exampleWidgets: o2 } = this.parts,
        h2;
      if (this.iframe)
        h2 = await G22({
          html: this.html,
    ...
    displayTestResults() {
      let { testResults: i3, testIndicator: l6 } = this.parts,
        s2 = this.testResults;
      ...
      let o2 = j1(
        ...
        `${s2.passed}/${s2.tests.length} tests passed`
      );
      i3.append(o2);
      for (let h2 of s2.tests) {
        let r2 = h2.passed ? "✓" : "✗",
          y2 = h2.passed ? "test-pass" : "test-fail",
          e2 = j1(
            { class: y2 },
            h4(r2 + " "),
            h2.name,
            h2.error ? h4({ style: { opacity: "0.7" } }, ` - ${h2.error}`) : ""
          );
        i3.append(e2);
      }
      this.classList.toggle("-test-passed", s2.failed === 0),
        this.classList.toggle("-test-failed", s2.failed > 0),
        (l6.title =
          s2.failed === 0
            ? `${s2.passed} tests passed`
            : `${s2.failed}/${s2.tests.length} tests failed`),
        this.updateTestResultsVisibility(),
        this.dispatchEvent(
          new CustomEvent("testcomplete", {
            bubbles: true,
            detail: { results: s2, element: this },
          })
        );
    }
    initFromElements(i3) {
...
`);
        if (["js", "html", "css", "test"].includes(s2)) {
          let h2 = o2
              .filter((y2) => y2.trim() !== "")
              .map((y2) => y2.match(/^\s*/)[0].length)
              .sort()[0],
            r2 = (h2 > 0 ? o2.map((y2) => y2.substring(h2)) : o2).join(`
`);
          this.setEditorValue(s2, r2);
        } else {
          let h2 = ["js", "html", "css", "test"].find((r2) =>
            l6.matches(`.language-${r2}`)
          );
          if (h2)
            this.setEditorValue(
              h2,
              h2 === "html" ? l6.innerHTML : l6.innerText
            );
        }
      }
    }
    showDefaultTab() {
      ...
      let { editors: i3 } = this.parts;
      if (this.js !== "") i3.value = 0;
      else if (this.html !== "") i3.value = 1;
      else if (this.css !== "") i3.value = 2;
      else if (this.test !== "") i3.value = 3;
    }
    render() {
      if ((super.render(), this.remoteId !== "")) {
        ...
        if (i3 !== null) {
          ...
          if (this.remoteKey !== l6.remoteKey) return;
          if (
            ((this.css = l6.css),
            (this.html = l6.html),
            (this.js = l6.js),
  ...
  if (r4)
    (document.title += " [code editor]"),
      ...
      document.body.append(Y2({ remoteId: r4 }));
  var { slot: x4 } = I;
  ...
  class A1 extends u {
    ...
    static styleSpec = {
      ":host": {
        display: "grid",
        gridTemplateColumns: `${kE.navWidth("50%")} ${kE.contentWidth("50%")}`,
        gridTemplateRows: "100%",
        position: "relative",
        margin: kE.margin("0 0 0 -100%"),
        transition: kE.sideNavTransition("0.25s ease-out"),
      },
      ":host slot": { position: "relative" },
      ":host slot:not([name])": { display: "block" },
      ':host slot[name="nav"]': { display: "block" },
    };
    onResize = () => {
      let { content: i3 } = this.parts,
        l6 = this.offsetParent;
      if (l6 === null) return;
      let s2 = this.value;
      if (
        ((this.compact = l6.offsetWidth < this.minSize),
        [...this.childNodes].find((h2) =>
          h2 instanceof Element ? h2.getAttribute("slot") !== "nav" : true
        ) === undefined)
      )
        (s2 = "compact/nav"),
        ...
        (s2 = "normal"),
          i3.classList.add("-tosi-sidenav-visible"),
        ...
        (i3.classList.remove("-tosi-sidenav-visible"),
      ...
      if (this.value !== s2) this.value = s2;
    };
    observer;
    connectedCallback() {
      super.connectedCallback(),
  ...
  var F1 = {
    pass: kE.testColorPass("#0a0"),
    fail: kE.testColorFail("#c00"),
    running: kE.testColorRunning("#fa0"),
  };
  var _i = {
    "@keyframes test-pulse": {
      "0%, 100%": { opacity: "1" },
    ...
    ".doc-link.-test-passed::after": { background: F1.pass },
  ...
  function l8(i3) {
    ...
    for (let c3 of l6) c3.testStatus = undefined;
    let e2 =
        document.location.search !== ""
          ? document.location.search.substring(1).split("&")[0]
          : l6[0]?.filename || "README.md",
      x3 = l6.find((c3) => c3.filename === e2) || l6[0],
      { app: n5 } = xE({ app: { docs: l6, currentDoc: x3, compact: false } }),
      p3 = {},
      d3,
      C3 = false,
      k3 = 0,
    ...
    let O = (c3) => {
        let f = p3[c3],
          g = n5.docs.find((B3) => B3.filename === c3);
        if (g) g.testStatus = f ? (f.passed ? "passed" : "failed") : undefined;
      },
      R3 = () => {
        if (w3 >= k3 && d3) {
      ...
      H =
        ...
        window.location.hostname === "127.0.0.1",
      F4 = (c3) => {
        let { results: f } = c3.detail,
          g = String(n5.currentDoc.filename);
        (p3[g] = {
          passed: f.failed === 0,
          tests: [...f.tests],
    ...
    (TE.docLink = {
      ...
      (TE.testStatus = {
        toDOM(c3, f) {
          if (
            (c3.classList.remove("-test-passed", "-test-failed"),
            f === "passed")
          )
            c3.classList.add("-test-passed");
          else if (f === "failed") c3.classList.add("-test-failed");
        },
      });
    let v1 = UE(() => {
        ...
        n5.docs.forEach((f) => {
          f.hidden =
            !f.title.toLocaleLowerCase().includes(c3) &&
            !f.text.toLocaleLowerCase().includes(c3);
        }),
          N(n5.docs);
      }),
      c0 = Vi({
      ...
      n4(
        {
          ...
          bind: {
            ...
            binding: {
              toDOM(c3, f) {
                (c3.style.display = f ? "" : "none"),
                  (c3.nextSibling.style.display = f ? "" : "none");
              },
            },
          },
          onClick() {
    ...
    if (o3)
      o1.push(
        e1(
          ...
          h2.tosijs
            ? t3.tosiUi({ style: { _xinIconSize: 40, marginRight: 10 } })
            : Z1(),
          Li(o3)
        )
      );
    if ((o1.push(Z1({ class: "elastic" })), h2.tosijs))
      o1.push(
        e1({ class: "iconic", title: "tosijs", target: "_blank" }, t3.tosi(), {
          href: h2.tosijs,
        })
      );
    if (h2.discord)
    ...
    if (h2.blog)
      o1.push(
        e1({ class: "iconic", title: "blog", target: "_blank" }, t3.blog(), {
          href: h2.blog,
        })
      );
    if (h2.github)
    ...
    if (h2.npm)
      o1.push(
        e1({ class: "iconic", title: "npmjs", target: "_blank" }, t3.npm(), {
          href: h2.npm,
        })
      );
    let e22 = R2(
            ...
            e1(
              {
                ...
                onClick(c3) {
                  let f = c3.target,
                    g = GM(c3.target),
                    B3 = c3.target.closest("tosi-sidenav");
                  B3.contentVisible = true;
                  let { href: V3 } = f;
                  window.history.pushState({ href: V3 }, "", V3),
                    (n5.currentDoc = g),
                    c3.preventDefault();
                  let D2 = String(g.filename),
                    b3 = p3[D2];
                  if (b3 && !b3.passed)
                    setTimeout(() => {
                      let L3 = document.querySelector(
              ...
              bind: {
                ...
                binding(c3, f) {
                  if (h2.github && f.path && f.path !== "README.md")
                    (c3.href = `${h2.github}/blob/main/${f.path}`),
                      (c3.style.display = "flex");
                  else c3.style.display = "none";
                },
              },
            },
            t3.github({ style: { _xinIconSize: 16 } }),
    ...
    let $3 = n4(
      { class: "test-widget", hidden: true, onClick: J4 },
      Z1({ part: "label" }, "Tests"),
      Z1({ class: "count", part: "count" }, "0")
    );
    e22.appendChild($3);
    let R1 = false;
    function p0() {
      (R1 = true),
        ($3.hidden = false),
        $3.classList.remove("-passed", "-failed"),
        $3.classList.add("-running"),
        d0();
    }
    function d0() {
      let c3 = $3.querySelector('[part="label"]'),
        f = $3.querySelector('[part="count"]'),
        g = Object.values(p3).reduce((V3, D2) => V3 + D2.totalPassed, 0),
        B3 = Object.values(p3).reduce((V3, D2) => V3 + D2.totalFailed, 0);
      if (c3)
        if (R1) c3.textContent = "Running";
        else if (B3 > 0) c3.textContent = "Failed";
        else if (g > 0) c3.textContent = "Passed";
        else c3.textContent = "Tests";
      if (f) f.textContent = B3 > 0 ? String(B3) : String(g);
    }
    function x22() {
      let c3 = Object.values(p3).reduce((f, g) => f + g.totalFailed, 0);
      if (R1 && w3 >= k3)
        if (((R1 = false), $3.classList.remove("-running"), c3 > 0))
          $3.classList.add("-failed"),
            $3.classList.remove("-passed"),
            ($3.hidden = false);
        else
          $3.classList.add("-passed"),
            $3.classList.remove("-failed"),
            ($3.hidden = false);
      d0();
    }
    function J4() {
      let c3 = Object.entries(p3).filter(([, g]) => !g.passed),
        f = [];
      for (let [g, B3] of c3) {
        let V3 = l6.find((b3) => b3.filename === g),
          D2 = B3.tests.filter((b3) => !b3.passed);
        for (let b3 of D2)
          f.push({
            caption: `${V3?.title || g}: ${b3.name}`,
            action: () => {
              let L3 = n5.docs.find((U3) => String(U3.filename) === g);
              if (L3)
                window.history.pushState({ href: `?${g}` }, "", `?${g}`),
                  (n5.currentDoc = L3),
                  setTimeout(() => {
                    let U3 = document.querySelector(
                      "tosi-example.-test-failed"
                    );
                    if (U3)
                      U3.scrollIntoView({
    ...
    function Q4() {
      ...
      for (let [B3, V3] of Object.entries(p3)) {
        ...
        if (
          ((f += V3.totalPassed), (g += V3.totalFailed), V3.tests.length > 0)
        ) {
          c3.push(`## ${b3}`), c3.push("");
      ...
      return (
        c3.unshift(`**Summary: ${f} passed, ${g} failed**`, ""),
        c3.join(`
`)
      );
    }
    ...
    let b4 = async () => {
        if (C3) return;
        if (!Y.enabled.value) return;
        C3 = true;
        let c3 = l6.filter((B3) => B3.text.includes("```test"));
        if (((k3 = c3.length), k3 > 0)) p0();
        if (k3 === 0) {
          if (d3) d3({ passed: 0, failed: 0, pages: {} }), (d3 = undefined);
          return;
        }
        ...
        for (let B3 of c3) {
          ...
          V3.appendChild(D2);
          let b3 = (U3) => {
            let { results: V1 } = U3.detail,
              L1 = p3[B3.filename];
            L1.tests.push(...V1.tests),
              (L1.totalPassed += V1.passed),
              (L1.totalFailed += V1.failed),
              (L1.passed = L1.totalFailed === 0),
              O(B3.filename),
              x22();
          };
          ...
          if (L3)
            (L3.body.innerHTML = ""),
              L3.body.appendChild(V3),
              await new Promise((U3) => setTimeout(U3, 500));
          P3(B3.filename);
        }
      ...
      f0 = () => {
        if (!Y.enabled.value) return;
        if (H) setTimeout(b4, 1000);
        else if (x3.text.includes("```test"))
          (k3 = 1), p0(), setTimeout(() => P3(x3.filename), 2000);
        else if (d3) d3({ passed: 0, failed: 0, pages: {} }), (d3 = undefined);
      };
  ...
  class m3 extends u {
    ...
    static styleSpec = {
      ...
      ":host ::slotted(*)": { position: "absolute" },
      ":host > :not(style,slot)": {
        boxSizing: "border-box",
        content: '" "',
        position: "absolute",
      ...
      ":host .icon-unlock": { opacity: 0.5 },
      ":host svg": { pointerEvents: "none" },
      ":host > *:hover": {
    ...
    static snappedCoords(i3, l6) {
      let { gridSize: s2 } = m3;
      return m3.snapToGrid || i3.shiftKey
        ? l6.map((o3) => Math.round(o3 / s2) * s2)
        : l6;
    }
    ...
    get locked() {
      ...
      let l6 = i3.style.right.match(/\d/) !== null,
        s2 = !l6 || i3.style.left.match(/\d/) !== null,
        o3 = i3.style.bottom.match(/\d/) !== null,
        h2 = !o3 || i3.style.top.match(/\d/) !== null;
      return { left: s2, top: h2, bottom: o3, right: l6 };
    }
    set locked(i3) {
      let { bottom: l6, right: s2 } = i3,
        { left: o3, top: h2 } = i3,
        r2 = this.parentElement,
        y2 = r2.offsetLeft,
        e2 = r2.offsetTop,
        x3 = r2.offsetWidth,
        n5 = r2.offsetHeight,
        p3 = r2.offsetParent.offsetWidth - y2 - x3,
        d3 = r2.offsetParent.offsetHeight - e2 - n5;
      if (
        ...
        o3 = true;
      if (!l6) h2 = true;
      if (o3) r2.style.left = y2 + "px";
      if (s2) r2.style.right = p3 + "px";
      if (o3 && s2) r2.style.width = "auto";
      else r2.style.width = x3 + "px";
      if (h2) r2.style.top = e2 + "px";
      if (l6) r2.style.bottom = d3 + "px";
      if (h2 && l6) r2.style.height = "auto";
      else r2.style.height = n5 + "px";
      this.queueRender();
    }
    get coords() {
      let {
    ...
    get right() {
      return (
        this.parentElement.offsetParent.offsetWidth - (this.left + this.width)
      );
    }
    get top() {
    ...
    get bottom() {
      return (
        this.parentElement.offsetParent.offsetHeight - (this.top + this.height)
      );
    }
    triggerChange = () => {
    ...
    adjustPosition = (i3) => {
      let { locked: l6 } = this;
      this.locked = l6;
      let s2 = this.parentElement,
        { top: o3, left: h2, bottom: r2, right: y2 } = this.coords;
      G2(i3, (e2, x3, n5) => {
        if ((([e2, x3] = m3.snappedCoords(n5, [e2, x3])), !isNaN(o3)))
          s2.style.top = o3 + x3 + "px";
        if (!isNaN(r2)) s2.style.bottom = r2 - x3 + "px";
        if (!isNaN(h2)) s2.style.left = h2 + e2 + "px";
        if (!isNaN(y2)) s2.style.right = y2 - e2 + "px";
        if (n5.type === "mouseup") return this.triggerChange(), true;
      });
    };
    resize = (i3) => {
      let l6 = this.parentElement,
        { locked: s2 } = this;
      ...
      G2(i3, (r2, y2, e2) => {
        let x3 = o3 - r2,
          n5 = h2 - y2;
        if (
          (([x3, n5] = m3.snappedCoords(e2, [x3, n5])),
          (l6.style.right = x3 + "px"),
          (l6.style.bottom = n5 + "px"),
          e2.type === "mouseup")
        )
    ...
    adjustSize = (i3) => {
      let l6 = this.parentElement,
        { locked: s2 } = this,
        o3 = i3.target.getAttribute("part");
      this.locked = Object.assign({
        left: true,
        right: true,
        top: true,
      ...
      let h2 = this[o3];
      G2(i3, (r2, y2, e2) => {
        let [x3] = m3.snappedCoords(e2, [
          h2 +
            (["left", "right"].includes(o3) ? r2 : y2) *
              (["right", "bottom"].includes(o3) ? -1 : 1),
        ]);
    ...
    adjustRotation = (i3) => {
      let { center: l6 } = this,
        { transformOrigin: s2 } = this.element.style;
      if (!s2) this.element.style.transformOrigin = "50% 50%";
      G2(i3, (o3, h2, r2) => {
        let { clientX: y2, clientY: e2 } = r2,
          x3 = y2 - l6.x,
          n5 = e2 - l6.y,
          p3 = n5 > 0 ? 90 : -90;
        if (x3 !== 0) p3 = (Math.atan2(n5, x3) * 180) / Math.PI;
        if (((p3 = m3.snappedAngle(r2, p3)), p3 === 0))
          (this.element.style.transformOrigin = ""),
            (this.element.style.transform = "");
        else this.element.style.transform = `rotate(${p3}deg)`;
        return this.triggerChange(), r2.type === "mouseup";
      });
    };
    toggleLock = (i3) => {
      let { locked: l6 } = this,
        s2 = i3.target.title.split(" ")[1];
      (l6[s2] = !l6[s2]),
        (this.locked = l6),
        this.queueRender(),
        i3.stopPropagation(),
    ...
    content = () => [
      ...
      W2({ part: "resize", style: { top: "100%", left: "100%" } }, t3.resize()),
      W2({ part: "rotate", style: { top: "50%", right: "0" } }, t3.refreshCw()),
      W2(
        {
          part: "lockLeft",
          title: "lock left",
    ...
    connectedCallback() {
      ...
      [i3, l6, s2, o3].forEach((C3) => {
        C3.addEventListener("mousedown", this.adjustSize, d3),
          C3.addEventListener("touchstart", this.adjustSize, d3);
      }),
        [h2, r2, y2, e2].forEach((C3) => {
          C3.addEventListener("click", this.toggleLock);
        }),
        n5.addEventListener("mousedown", this.resize, d3),
        x3.addEventListener("mousedown", this.adjustPosition, d3),
        p3.addEventListener("mousedown", this.adjustRotation, d3),
        n5.addEventListener("touchstart", this.resize, d3),
        x3.addEventListener("touchstart", this.adjustPosition, d3),
        p3.addEventListener("touchstart", this.adjustRotation, d3);
    }
    render() {
      if ((super.render(), !this.parentElement)) return;
      let {
          lockLeft: i3,
          lockRight: l6,
          lockTop: s2,
        ...
        { left: h2, right: r2, top: y2, bottom: e2 } = this.locked;
      i3.toggleAttribute("locked", h2),
        l6.toggleAttribute("locked", r2),
        s2.toggleAttribute("locked", y2),
        o3.toggleAttribute("locked", e2);
    }
  }
  ...
  var f4 = {
    contains: {
      ...
      makeTest: (i3) => {
        return (
          (i3 = i3.toLocaleLowerCase()),
          (l6) => String(l6).toLocaleLowerCase().includes(i3)
        );
      },
    },
    hasTags: {
      ...
      makeTest: (i3) => {
        let l6 = i3
          .split(/[\s,]/)
          .map((s2) => s2.trim().toLocaleLowerCase())
          .filter((s2) => s2 !== "");
        return (s2) =>
          Array.isArray(s2) && l6.find((o3) => !s2.includes(o3)) === undefined;
      },
    },
    doesNotHaveTags: {
    ...
    equals: {
      ...
      makeTest: (i3) => {
        if (isNaN(Number(i3)))
          return (
            (i3 = String(i3).toLocaleLowerCase()),
            (s2) => String(s2).toLocaleLowerCase() === i3
          );
        let l6 = Number(i3);
    ...
    greaterThan: {
      ...
      makeTest: (i3) => {
        ...
        return (
          (i3 = i3.toLocaleLowerCase()),
          (l6) => String(l6).toLocaleLowerCase() > i3
        );
      },
    },
    truthy: {
  ...
  class C4 extends u {
    ...
    content = () => [
      X3({ part: "haystack" }),
      X3({ part: "condition" }),
      bi({ part: "needle", type: "search" }),
      Gi({ part: "padding" }),
      E2({ part: "remove", title: "delete" }, t3.trash()),
    ];
    filter = Ui;
    get state() {
    ...
    buildFilter = () => {
      let { haystack: i3, condition: l6, needle: s2 } = this.parts,
        o3 = l6.value.startsWith("~"),
        h2 = o3 ? l6.value.slice(1) : l6.value,
        r2 = this.filters[h2];
      s2.hidden = r2.needsValue === false;
      let y2 =
          r2.needsValue === false
            ? r2.makeTest(undefined)
            : r2.makeTest(s2.value),
        e2 = i3.value,
        x3;
      if (e2 !== "*") x3 = o3 ? (d3) => !y2(d3[e2]) : (d3) => y2(d3[e2]);
      else
        x3 = o3
          ? (d3) => Object.values(d3).find((C3) => !y2(C3)) !== undefined
          : (d3) => Object.values(d3).find((C3) => y2(C3)) !== undefined;
      let n5 = r2.needsValue !== false ? ` "${s2.value}"` : "",
        p3 = `${p4(i3)} ${p4(l6)}${n5}`;
      (this.filter = { description: p3, test: x3 }),
        this.parentElement?.dispatchEvent(new Event("change"));
    };
    connectedCallback() {
      super.connectedCallback();
      let { haystack: i3, condition: l6, needle: s2, remove: o3 } = this.parts;
      i3.addEventListener("change", this.buildFilter),
        l6.addEventListener("change", this.buildFilter),
        s2.addEventListener("input", this.buildFilter),
    ...
    render() {
      ...
      if (
        ((i3.options = [
          { caption: "any field", value: "*" },
          ...this.fields.map((o3) => o3.prop),
        ]),
        (l6.options = Object.keys(this.filters)
          .map((o3) => {
            let h2 = this.filters[o3];
            return h2.negative !== undefined
              ? [
                  { caption: h2.caption, value: o3 },
                  { caption: h2.negative, value: "~" + o3 },
                ]
              : { caption: h2.caption, value: o3 };
          })
          .flat()),
        ...
        i3.value = this.haystack;
      if (this.condition !== "") l6.value = this.condition;
      if (this.needle !== "") s2.value = this.needle;
      this.buildFilter();
    }
  }
  var D2 = C4.elementCreator({
    tag: "tosi-filter-part",
    styleSpec: {
      ":host": { display: "flex" },
      ":host .tosi-icon:": { verticalAlign: "middle", pointerEvents: "none" },
      ':host [part="haystack"], :host [part="condition"]': { flex: "1" },
      ':host [part="needle"]': { flex: 2 },
  ...
  class a4 extends u {
    ...
    set state(i3) {
      let { fields: l6, filters: s2 } = this,
        { filterContainer: o3 } = this.parts;
      o3.textContent = "";
      for (let h2 of i3) o3.append(D2({ fields: l6, filters: s2, ...h2 }));
    }
    filter = t4;
    description = c4;
    addFilter = () => {
      let { fields: i3, filters: l6 } = this,
        { filterContainer: s2 } = this.parts;
    ...
    content = () => [
      ...
      Qi({ part: "filterContainer" }),
      E2({ part: "reset", title: "reset filter", onClick: this.reset }, t3.x()),
    ];
    filters = f4;
    reset = () => {
      let { fields: i3, filters: l6 } = this,
    ...
    buildFilter = () => {
      ...
      let l6 = [...i3.children].map((o3) => o3.filter),
        s2 = l6.map((o3) => o3.test);
      (this.description = l6.map((o3) => o3.description).join(", ")),
        (this.filter = (o3) =>
          o3.filter((h2) => s2.find((r2) => r2(h2) === false) === undefined)),
        this.dispatchEvent(new Event("change"));
    };
    connectedCallback() {
      super.connectedCallback();
  ...
  var C8 = a4.elementCreator({
    ...
    styleSpec: {
      ...
      ':host [part="haystack"]': { _fieldWidth: "100px" },
      ':host [part="condition"]': { _fieldWidth: "60px" },
      ':host [part="needle"]': { _fieldWidth: "80px" },
  ...
  function n1(i3, l6, s2) {
    if (s2 !== "" && s2 !== false) i3.setAttribute(l6, s2);
    else i3.removeAttribute(l6);
  }
  function Wi(i3) {
    switch (i3.type) {
      case "checkbox":
        return i3.checked;
      case "radio": {
        let l6 = i3.parentElement?.querySelector(
          `input[type="radio"][name="${i3.name}"]:checked`
        );
      ...
      default:
        return Array.isArray(i3.value) && i3.value.length === 0
          ? null
          : i3.value;
    }
  }
  function k4(i3, l6) {
    ...
    else if (l6 != null || i3.value != null) i3.value = String(l6 || "");
  }
  ...
  class u22 extends u {
        ...
        switch (this.type) {
          ...
          case "range":
            if (this.fixedPrecision > -1)
              (s2.value = Number(s2.value).toFixed(this.fixedPrecision)),
                (o3.fields[this.key] = Number(s2.value));
            else o3.fields[this.key] = Number(s2.value);
            break;
          default:
    ...
    connectedCallback() {
      super.connectedCallback();
      let { input: i3, valueHolder: l6 } = this.parts;
      l6.addEventListener("change", this.handleChange),
        i3.addEventListener("change", this.handleChange, true);
    }
    render() {
      if (this.valueChanged) {
        this.valueChanged = false;
      ...
      if (this.type === "text") {
        ...
        if (this.placeholder) h2.setAttribute("placeholder", this.placeholder);
        i3.append(h2);
      } else if (this.type === "color")
        (i3.textContent = ""), i3.append(j0({ value: this.value }));
      else if (i3.children.length === 0) {
        if (
          (n1(s2, "placeholder", this.placeholder),
          n1(s2, "type", this.type),
          n1(s2, "pattern", this.pattern),
          n1(s2, "min", this.min),
          n1(s2, "max", this.max),
          this.step)
        )
      ...
      if (
        (k4(s2, this.value),
        k4(i3.children[0], this.value),
        this.prefix
          ? o3.setAttribute("prefix", this.prefix)
          : o3.removeAttribute("prefix"),
        this.suffix
          ? o3.setAttribute("suffix", this.suffix)
          : o3.removeAttribute("suffix"),
        s2.classList.toggle("hidden", i3.children.length > 0),
        i3.children.length > 0)
      )
        s2.setAttribute("tabindex", "-1");
      else s2.removeAttribute("tabindex");
      (i3.style.display = i3.children.length === 0 ? "none" : ""),
        n1(s2, "required", !this.optional);
    }
  }
  ...
  class m22 extends u {
    ...
    get isValid() {
      return (
        [...this.querySelectorAll("*")]
          .filter((l6) => l6.required !== undefined)
          .find((l6) => !l6.reportValidity()) === undefined
      );
    }
    static styleSpec = {
      ":host": { display: "flex", flexDirection: "column" },
      ":host::part(header), :host::part(footer)": { display: "flex" },
      ":host::part(content)": {
        display: "flex",
        flexDirection: "column",
        overflow: "hidden auto",
    ...
    content = [
      I22({ part: "header", name: "header" }),
      Xi({ part: "form" }, I22({ part: "content" })),
      I22({ part: "footer", name: "footer" }),
    ];
    getField = (i3) => {
    ...
    get fields() {
      if (typeof this.value === "string")
        try {
          ...
          console.log(
            "<tosi-form> could not use its value, expects valid JSON"
          ),
            (this.value = {});
        }
      let { getField: i3 } = this,
        l6 = this.dispatchEvent.bind(this);
    ...
    set fields(i3) {
      ...
      for (let s2 of l6) s2.value = i3[s2.key];
    }
    submit = () => {
  ...
  var Ri = {
    ':host [part="field"]': {
      ...
      display: "flex",
      alignItems: "center",
      gap: kE.prefixSuffixGap("8px"),
    },
    ':host [part="field"][prefix]::before': { content: "attr(prefix)" },
    ':host [part="field"][suffix]::after': { content: "attr(suffix)" },
    ':host [part="field"] > *, :host [part="input"] > *': { width: "100%" },
    ":host textarea": { resize: "none" },
    ':host input[type="checkbox"]': { width: "fit-content" },
    ":host .hidden": {
      position: "absolute",
      pointerEvents: "none",
      opacity: 0,
      ...
      .map((l6) => {
        ...
        return {
          ...
          buttons: h2
            ...
            .filter((r2) => r2.pressed || r2.value !== 0)
            .reduce((r2, y2) => {
  ...
  function j8() {
    ...
    return i3.length === 0
      ? "no active gamepads"
      : i3.map(({ id: l6, axes: s2, buttons: o3 }) => {
          let h2 = s2.map((y2) => y2.toFixed(2)).join(" "),
            r2 = Object.keys(o3)
              .map((y2) => `[${y2}](${o3[Number(y2)].toFixed(2)})`)
              .join(" ");
          return `${l6}
${h2}
${r2}`;
...
`);
  ...
  function A8(i3) {
    let l6 = {};
    return (
      i3.input.onControllerAddedObservable.add((s2) => {
        s2.onMotionControllerInitObservable.add((o3) => {
          ...
          o3.getComponentIds().forEach((y2) => {
            ...
            if (
              ...
              e2.onButtonStateChangedObservable.add(() => {
                h2[y2].pressed = e2.pressed;
              }),
              e2.onAxisValueChangedObservable)
            )
              (h2[y2].axes = []),
                e2.onAxisValueChangedObservable.add((x3) => {
                  h2[y2].axes = x3;
                });
          }),
            (l6[o3.handedness] = h2);
  ...
  function F8(i3) {
    if (i3 === undefined || Object.keys(i3).length === 0) return "no xr inputs";
    return Object.keys(i3).map((l6) => {
      let s2 = i3[l6],
        o3 = Object.keys(s2)
          .filter((h2) => s2[h2].pressed)
          .join(" ");
      return `${l6}
${o3}`;
    }).join(`
`);
    ...
    render() {
      ...
      if (this._map) {
        if (this.coords !== this._lastCoords) {
          ...
          this._map.setCenter([r2, h2]),
            this._map.setZoom(y2),
      ...
      let { div: i3 } = this.parts,
        [l6, s2, o3] = this.coords.split(",").map((h2) => Number(h2));
      (this._lastCoords = this.coords),
        (this._lastStyle = this.mapStyle),
        q1.mapboxAvailable.then(({ mapboxgl: h2 }) => {
          console.log(
            "%cmapbox may complain about missing css -- don't panic!",
            "background: orange; color: black; padding: 0 5px;"
          ),
            (h2.accessToken = this.token),
            (this._map = new h2.Map({
              container: i3,
              style: this.mapStyle,
              zoom: o3,
              center: [s2, l6],
            ...
            this._map.on("moveend", () => {
              let r2 = this._map.getCenter(),
                y2 = this._map.getZoom(),
                e2 = `${r2.lat.toFixed(6)},${r2.lng.toFixed(6)},${y2.toFixed(
                  1
                )}`;
              if (e2 !== this.value) {
                if (this.internals) this.internals.setFormValue(e2);
              }
            });
        });
    }
  ...
  class M4 extends u {
    ...
    static initAttributes = {
      ...
      year: NaN,
      weekStart: 0,
      minDate: K1(new Date().getFullYear() - 100, 1, 1)
        .toISOString()
        .split("T")[0],
      maxDate: K1(new Date().getFullYear() + 10, 12, 31)
        .toISOString()
        .split("T")[0],
      selectable: false,
      multiple: false,
    ...
    get months() {
      return sl.map((i3) => ({
        caption: K1(2025, i3, 1).toString().split(" ")[1],
        value: String(i3),
      }));
    }
    get years() {
      let i3 = Number(this.minDate.split("-")[0]),
        l6 = Number(this.maxDate.split("-")[0]),
        s2 = [];
      for (let o3 = i3; o3 <= l6; o3++) s2.push(String(o3));
      return s2;
    }
    monthChanged = (i3, l6) => {};
    gotoMonth(i3, l6) {
    ...
    selectDate = (i3) => {
      if (((this.#i = i3), this.range)) {
        if (!this.to) this.selectedDays = [i3, i3];
        else if (this.from === i3 && this.to === i3) this.selectedDays = [];
        else if (this.from === i3) this.from = this.to;
        else if (this.to === i3) this.to = this.from;
        else if (i3 < this.from) this.from = i3;
        else if (i3 > this.to) this.to = i3;
        else this.to = i3;
        this.value = `${this.from},${this.to}`;
      } else if (this.multiple) {
        if (this.selectedDays.includes(i3))
          this.selectedDays.splice(this.selectedDays.indexOf(i3), 1);
        else this.selectedDays.push(i3), this.selectedDays.sort();
        this.value = this.selectedDays.join(",");
      } else if (this.selectable)
        if (this.selectedDays.includes(i3))
          (this.value = ""), (this.selectedDays = []);
        else (this.value = i3), (this.selectedDays = [i3]);
    };
    nextMonth = () => {
      if (this.month < 12) this.gotoMonth(this.year, this.month + 1);
      else this.gotoMonth(this.year + 1, 1);
    };
    previousMonth = () => {
      if (this.month > 1) this.gotoMonth(this.year, this.month - 1);
      else this.gotoMonth(this.year - 1, 12);
    };
    checkDay = (i3) => {
      if (!this.range) return this.selectedDays.includes(i3);
      else if (this.range) return this.from && i3 >= this.from && i3 <= this.to;
      return false;
    };
    dateMenuItem = (i3, l6 = "") => {
      return (
        ...
        {
          caption: l6 || i3,
          enabled: () => !i3.startsWith(`${this.year}-${i0(this.month)}-`),
          action: () => {
            this.gotoDate(i3);
          },
        }
    ...
    jumpMenu = () => {
      J2({
        ...
        menuItems: [
          ...
          ...this.selectedDays.map((i3) => this.dateMenuItem(i3)),
        ],
      });
    };
    content = () => [
      S2(
        { part: "header" },
        T22(
          { part: "previous", onClick: this.previousMonth },
          t3.chevronLeft()
        ),
        h2({ style: { flex: "1" } }),
        T22({ part: "jump", onClick: this.jumpMenu }, t3.calendar()),
        X3({ part: "month", options: this.months, onChange: this.setMonth }),
        X3({ part: "year", options: [this.year], onChange: this.setMonth }),
        h2({ style: { flex: "1" } }),
        T22({ part: "next", onClick: this.nextMonth }, t3.chevronRight())
      ),
      S2({ part: "week" }),
      S2({ part: "days" }),
    ];
    ...
    connectedCallback() {
      ...
      if (isNaN(this.month)) this.month = i3.getMonth() + 1;
      if (isNaN(this.year)) this.year = i3.getFullYear();
    }
    days = [];
    render() {
      super.render();
      ...
      let e2 = K1(this.year, this.month, 1),
        x3 = new Date(
          e2.valueOf() - ((7 + e2.getDay() - this.weekStart) % 7) * X1
        ),
        n5 = this.month === 12 ? 1 : this.month + 1,
        p3 = new Date(
          K1(this.year + (this.month === 12 ? 1 : 0), n5, 1).valueOf() - X1
        ),
        d3 = new Date(
          p3.valueOf() +
            ((this.weekStart * 2 + 5 + this.endDay - p3.getDay()) % 7) * X1
        ),
        C3 = ll.map(
          (H) => new Date(x3.valueOf() + H * X1).toString().split(" ")[0]
        );
      this.days = [];
      let k3 = new Date().toISOString().split("T")[0];
      for (let H = x3.valueOf(); H <= d3.valueOf(); H += X1) {
        let F4 = new Date(H),
          P3 = F4.toISOString().split("T")[0];
        this.days.push({
          date: F4,
          selected: false,
          inMonth: F4.getMonth() + 1 === this.month,
          isToday: P3 === k3,
          isWeekend: F4.getDay() % 6 === 0,
          inRange: !!(this.from && P3 >= this.from && P3 <= this.to),
        });
      }
      (o3.value = String(this.month)),
        (h3.value = String(this.year)),
        (o3.disabled =
          h3.disabled =
          s2.disabled =
          r2.disabled =
        ...
        (h3.options = this.years),
        (i3.textContent = ""),
        i3.append(...C3.map((H) => h2({ class: "day" }, H))),
        (l6.textContent = "");
      let w3 = null,
        { to: O, from: R3 } = this;
      l6.append(
        ...this.days.map((H) => {
          let F4 = ["date"];
          if (H.inMonth) F4.push("in-month");
          if (H.isToday) F4.push("today");
          let P3 = H.date.toISOString().split("T")[0];
          if (this.checkDay(P3)) F4.push("checked");
          if ((F4.push(H.isWeekend ? "weekend" : "weekday"), this.range)) {
            if (O === P3) F4.push("range-end");
            if (R3 === P3) F4.push("range-start");
          }
          let v1 = h2(
            {
              class: F4.join(" "),
  ...
  var X8 = M4.elementCreator({
    ...
    styleSpec: {
      ...
      ":host[disabled]": {
        pointerEvents: "none",
        opacity: kE.disabledOpacity(0.6),
      },
      ...
      ":host .today": {
        background: kE.monthTodayBackground("transparent"),
        boxShadow: kE.monthTodayShadow("none"),
        backdropFilter: kE.monthTodayBackdropFilter("brightness(0.9)"),
        fontWeight: kE.monthTodayFontWeight("800"),
      },
      ":host .day, :host .date": {
        padding: 5,
        display: "flex",
      ...
      ":host .day": {
        color: kE.monthDayColor("hotpink"),
        background: kE.monthDayBackground("white"),
        fontWeight: kE.monthDayFontWeight("800"),
      },
      ":host .date": { cursor: "default" },
      ":host .weekend": { background: kE.monthWeekendBackground("#eee") },
      ":host .date:not(.in-month)": { opacity: 0.5 },
      ":host .date.checked": {
        color: kE.monthDateCheckedColor("white"),
        background: kE.monthDateCheckedBackground("hotpink"),
      },
      ":host:not([range]) .date.checked": {
        borderRadius: kE.monthDateCheckedBorderRadius("10px"),
      },
      ":host .range-start": {
        borderTopLeftRadius: kE.monthDateCheckedBorderRadius("10px"),
        borderBottomLeftRadius: kE.monthDateCheckedBorderRadius("10px"),
      },
      ":host .range-end": {
        borderTopRightRadius: kE.monthDateCheckedBorderRadius("10px"),
        borderBottomRightRadius: kE.monthDateCheckedBorderRadius("10px"),
      },
    },
  });
  ...
  class k1 extends u {
    ...
    static styleSpec = {
      ...
      ":host *": { color: fM.notificationTextColor },
      ":host .note": {
        display: "grid",
      ...
      ":host .note .icon": { stroke: fM.notificationAccentColor },
      ":host .note button": {
        display: "flex",
      ...
      ":host .note button:hover svg": { stroke: fM.notificationAccentColor },
      ...
      ":host .note.closing": { opacity: 0, zIndex: 0 },
    };
    static removeNote(i3) {
      i3.classList.add("closing"),
        (i3.style.marginBottom = -i3.offsetHeight + "px");
      let l6 = () => {
        i3.remove();
      };
    ...
    static post(i3) {
      let {
        ...
        typeof i3 === "string" ? { message: i3 } : i3
      );
      if (!this.singleton) this.singleton = z4();
      let x3 = this.singleton;
      document.body.append(x3), (x3.style.zIndex = String(B1() + 1));
      let n5 = e2 || rl[o3],
        p3 = r2 || o3 === "progress" ? I.progress() : {},
        d3 = () => {
          if (h3) h3();
          k1.removeNote(w3);
        },
        C3 =
          y2 instanceof SVGElement
            ? y2
            : y2
            ? t3[y2]({ class: "icon" })
            : t3.info({ class: "icon" }),
        ...
        w3 = l0(
          {
            class: `note ${o3}`,
            role: k3 ? "alert" : "status",
            ariaLive: k3 ? "assertive" : "polite",
            style: { _notificationAccentColor: n5 },
          },
          C3,
          l0({ class: "message" }, l0(l6), p3),
          hl(
            {
              class: "close",
              title: "close",
      ...
      if (
        (x3.shadowRoot.append(w3),
        p3 instanceof HTMLProgressElement && r2 instanceof Function)
      ) {
        ...
        let O = setInterval(() => {
          ...
          let R3 = r2();
          if (((p3.value = R3), R3 >= 100)) k1.removeNote(w3);
        }, 1000);
      }
      if (s2 > 0)
        setTimeout(() => {
  ...
  var xl = async (i3, l6 = "SHA-1") => {
    let o3 = new TextEncoder().encode(i3),
      h3 = await crypto.subtle.digest(l6, o3);
    return Array.from(new Uint8Array(h3))
      .map((e2) => e2.toString(16).padStart(2, "0"))
      .join("");
  };
  var nl = async (i3) => {
    let l6 = await xl(i3),
      s2 = await fetch(`https://weakpass.com/api/v1/search/${l6}`);
    if (s2.ok) {
  ...
  class o0 extends u {
    ...
    strength(i3) {
      return (
        (this.issues = {
          tooShort: i3.length < this.minLength,
          short: i3.length < this.goodLength,
          noUpper: !i3.match(/[A-Z]/),
          noLower: !i3.match(/[a-z]/),
          noNumber: !i3.match(/[0-9]/),
          noSpecial: !i3.match(/[^a-zA-Z0-9]/),
        }),
        this.issues.tooShort
          ? 0
          : Object.values(this.issues).filter((l6) => !l6).length - 1
      );
    }
    async isBreached() {
      let i3 = this.querySelector("input")?.value;
      if (!i3 || typeof i3 !== "string") return true;
      return await nl(i3);
    }
    updateIndicator = (i3) => {
      let { level: l6, description: s2 } = this.parts,
        o3 = this.indicatorColors.split(","),
        h3 = this.descriptionColors.split(","),
        r2 = this.strength(i3);
      if (this.value !== r2)
        (this.value = r2), this.dispatchEvent(new Event("change"));
      (l6.style.width = `${(r2 + 1) * 16.67}%`),
        this.style.setProperty("--indicator-color", o3[r2]),
        this.style.setProperty("--description-color", h3[r2]),
        (s2.textContent = this.strengthDescriptions[r2]);
    };
    update = (i3) => {
    ...
    content = () => [
      tl({ onInput: this.update }),
      s0({ part: "meter" }, s0({ part: "level" }), s0({ part: "description" })),
    ];
    render() {
      super.render();
  ...
  var cl = o0.elementCreator({
    ...
    styleSpec: {
      ...
      ":host xin-slot": { display: "flex" },
      ':host [part="meter"]': {
        display: "block",
        position: "relative",
        height: kE.meterHeight("24px"),
        background: kE.indicatorBg("white"),
        borderRadius: kE.meterRadius("4px"),
        boxShadow: kE.meterShadow(`inset 0 0 0 2px ${fM.indicatorColor}`),
      },
      ':host [part="level"]': {
        height: kE.levelHeight("20px"),
        content: '" "',
        display: "inline-block",
        width: 0,
        transition: "0.15s ease-out",
        background: fM.indicatorColor,
        margin: kE.levelMargin("2px"),
        borderRadius: kE.levelRadius("2px"),
      },
      ':host [part="description"]': {
        ...
        height: kE.meterHeight("24px"),
        lineHeight: kE.meterHeight("24px"),
        textAlign: "center",
      },
    },
  });
  ...
  class r0 extends u {
    ...
    static styleSpec = {
      ...
      ":host::part(empty)": { pointerEvents: "none" },
      ":host::part(filled)": {
        position: "absolute",
        left: 0,
      ...
      ":host svg:hover": { transform: "scale(1)" },
      ":host svg:active": { transform: "scale(1.1)" },
    };
    content = () =>
      h0({ part: "container" }, h0({ part: "empty" }), h0({ part: "filled" }));
    displayValue(i3) {
      let { empty: l6, filled: s2 } = this.parts,
        h3 =
          Math.round((typeof i3 === "string" ? 0 : i3 || 0) / this.step) *
          this.step;
      s2.style.width = (h3 / this.max) * l6.offsetWidth + "px";
    }
    update = (i3) => {
      if (this.readonly) return;
      let { empty: l6 } = this.parts,
        s2 =
          i3 instanceof MouseEvent
            ? i3.pageX - l6.getBoundingClientRect().x
            : 0,
        o3 = Math.min(
          Math.max(
            this.min,
            Math.round(
              ((s2 / l6.offsetWidth) * this.max) / this.step + this.step * 0.5
            ) * this.step
          ),
          this.max
        );
      if (i3.type === "click") this.value = o3;
      else if (i3.type === "mousemove") this.displayValue(o3);
      else this.displayValue(this.value || 0);
    };
    handleKey = (i3) => {
    ...
    connectedCallback() {
      ...
      (i3.tabIndex = 0),
        i3.addEventListener("mousemove", this.update, true),
        i3.addEventListener("mouseleave", this.update),
        i3.addEventListener("blur", this.update),
        i3.addEventListener("click", this.update),
        i3.addEventListener("keydown", this.handleKey);
    }
    _renderedIcon = "";
    render() {
      super.render();
      ...
      if ((this.style.setProperty("--tosi-icon-size", i3), this.readonly))
        this.role = "image";
      else this.role = "slider";
      (this.ariaLabel = `rating ${this.value} out of ${this.max}`),
        (this.ariaValueMax = String(this.max)),
        (this.ariaValueMin = String(this.min)),
        (this.ariaValueNow =
          this.value === "" ? String(-1) : String(this.value));
      let { empty: l6, filled: s2 } = this.parts;
      if (
        (l6.classList.toggle("hollow", this.hollow),
        l6.style.setProperty("--tosi-icon-fill", this.emptyFill),
        l6.style.setProperty("--tosi-icon-stroke", this.emptyStroke),
        s2.style.setProperty("--tosi-icon-fill", this.ratingFill),
        s2.style.setProperty("--tosi-icon-stroke", this.ratingStroke),
  ...
  var Ml = [
    { caption: "Title", tagType: "H1" },
    { caption: "Heading", tagType: "H2" },
    { caption: "Subheading", tagType: "H3" },
    { caption: "Minor heading", tagType: "H4" },
    { caption: "Body", tagType: "P" },
    { caption: "Code Block", tagType: "PRE" },
  ];
  function A4(i3 = Ml) {
    return X3({
      ...
      slot: "toolbar",
      class: "block-style",
      options: i3.map(({ caption: l6, tagType: s2 }) => ({
        caption: l6,
        value: `formatBlock,${s2}`,
      })),
    });
  }
  function Y1(i3 = "10px") {
  ...
  var Bl = () => [
    S3("left-justify", "justifyLeft", t3.alignLeft()),
    S3("center", "justifyCenter", t3.alignCenter()),
    S3("right-justify", "justifyRight", t3.alignRight()),
    Y1(),
    S3("bullet list", "insertUnorderedList", t3.listBullet()),
    S3("numbered list", "insertOrderedList", t3.listNumber()),
    Y1(),
    S3("indent", "indent", t3.indent()),
    S3("indent", "outdent", t3.outdent()),
  ];
  var F4 = () => [
    S3("bold", "bold", t3.fontBold()),
    S3("italic", "italic", t3.fontItalic()),
    S3("underline", "underline", t3.fontUnderline()),
  ];
  var zl = () => [A4(), Y1(), ...F4()];
  ...
  class y0 extends u {
    ...
    set value(i3) {
      ...
      if (((this._value = i3), this.isInitialized)) {
        if (this.parts.doc.innerHTML !== i3) this.parts.doc.innerHTML = i3;
      }
      if (l6 !== i3 && this.internals) this.internals.setFormValue(i3);
    }
    blockElement(i3) {
    ...
    get selectedBlocks() {
      let { doc: i3 } = this.parts,
        l6 = window.getSelection();
      if (l6 === null) return [];
      let s2 = [];
      for (let o3 = 0; o3 < l6.rangeCount; o3++) {
        let h3 = l6.getRangeAt(o3);
        if (!i3.contains(h3.commonAncestorContainer)) continue;
        let r2 = this.blockElement(h3.startContainer),
          y2 = this.blockElement(h3.endContainer);
        s2.push(r2);
        while (r2 !== y2 && r2 !== null)
    ...
    handleSelectChange = (i3) => {
      if (this._updatingBlockStyle) return;
    ...
    doCommand(i3) {
      ...
      let l6 = i3.split(",");
      console.log("execCommand", l6[0], false, ...l6.slice(1)),
        document.execCommand(l6[0], false, ...l6.slice(1));
    }
    updateBlockStyle() {
      ...
      (l6 = [...new Set(l6)]),
        ...
        (i3.value = l6.length === 1 ? `formatBlock,${l6[0]}` : ""),
    ...
    connectedCallback() {
      ...
      (this.isInitialized = true),
        ...
        i3.addEventListener("input", this.handleInput),
        this.updateValidity(),
        document.addEventListener("selectionchange", (s2) => {
  ...
  var ds = y0;
  var jl = y0.elementCreator({
    tag: "tosi-rich-text",
    styleSpec: {
      ":host": { display: "flex", flexDirection: "column", height: "100%" },
      ':host [part="toolbar"]': {
        padding: 4,
        display: "flex",
        gap: "0px",
      ...
      ':host [part="toolbar"] > button': { _xinIconSize: 18 },
    },
  });
  var fs = gE(
    (...i3) => jl(...i3),
  ...
  class N1 extends u {
    ...
    set choices(i3) {
      if (typeof i3 === "string") this._choices = N1.parseChoicesString(i3);
      else this._choices = i3;
      this.queueRender();
    }
    static parseChoicesString(i3) {
      return i3
        ...
        .filter((l6) => l6.trim() !== "")
        .map((l6) => {
          let [s2, o3] = l6.split("=").map((e2) => e2.trim()),
            [h3, r2] = (o3 || s2).split(":").map((e2) => e2.trim()),
            y2 = r2 ? t3[r2]() : "";
    ...
    content = () => [
      Ll(),
      Vl({ part: "options" }, q4({ part: "custom", hidden: true })),
    ];
    static styleSpec = {
      ":host": {
        display: "inline-flex",
        gap: kE.segmentedOptionGap("8px"),
        alignItems: kE.segmentedAlignItems("center"),
      },
      ":host, :host::part(options)": {
        flexDirection: kE.segmentedDirection("row"),
      },
      ":host label": {
        display: "inline-grid",
        alignItems: "center",
        gap: kE.segmentedOptionGap("8px"),
        gridTemplateColumns: kE.segmentedOptionGridColumns("0px 24px 1fr"),
        padding: kE.segmentedOptionPadding("4px 12px"),
        font: kE.segmentedOptionFont("16px"),
      },
      ":host label:focus": {
        outline: "none",
        boxShadow: kE.segmentedFocusShadow(
          `inset 0 0 0 2px ${kE.segmentedOptionCurrentBackground("#44a")}`
        ),
        borderRadius: kE.segmentedOptionsBorderRadius("8px"),
      },
      ":host label:has(:checked)": {
        color: kE.segmentedOptionCurrentColor("#eee"),
        background: kE.segmentedOptionCurrentBackground("#44a"),
      },
      ":host label:has(:checked):focus": {
        boxShadow: kE.segmentedCurrentFocusShadow(
          `inset 0 0 0 2px ${kE.segmentedOptionCurrentColor("#eee")}`
        ),
      },
      ":host svg": {
        height: kE.segmentOptionIconSize("16px"),
        stroke: kE.segmentedOptionIconColor("currentColor"),
      },
      ":host label.no-icon": {
        gap: 0,
        gridTemplateColumns: kE.segmentedOptionGridColumns("0px 1fr"),
      },
      ':host input[type="radio"], :host input[type="checkbox"]': {
        visibility: kE.segmentedInputVisibility("hidden"),
      },
      ":host::part(options)": {
        display: "flex",
        borderRadius: kE.segmentedOptionsBorderRadius("8px"),
        background: kE.segmentedOptionsBackground("#fff"),
        color: kE.segmentedOptionColor("#222"),
        overflow: "hidden",
        alignItems: kE.segmentedOptionAlignItems("stretch"),
      },
      ":host::part(custom)": {
        padding: kE.segmentedOptionPadding("4px 12px"),
        color: kE.segmentedOptionCurrentColor("#eee"),
      ...
      ":host::part(custom)::placeholder": {
        color: kE.segmentedOptionCurrentColor("#eee"),
        opacity: kE.segmentedPlaceholderOpacity(0.75),
      },
    };
    valueChanged = false;
    handleChange = () => {
    ...
    connectedCallback() {
      ...
      if (this.name === "") this.name = this.instanceId;
      if (
        (l6.addEventListener("change", this.handleChange),
        l6.addEventListener("keydown", this.handleKey),
    ...
    get isOtherValue() {
      return Boolean(
        this.value === "" ||
          (this.value &&
            !this._choicesWithOther.find((i3) => i3.value === this.value))
      );
    }
    render() {
      ...
      let { options: i3, custom: l6 } = this.parts;
      i3.textContent = "";
      let s2 = this.multiple ? "checkbox" : "radio",
        { values: o3, isOtherValue: h3 } = this;
      if (
        (i3.append(
          ...this._choicesWithOther.map((r2) => {
            return _l(
              { tabindex: 0 },
              q4({
                type: s2,
                name: this.name,
                value: r2.value,
                checked: o3.includes(r2.value) || (r2.value === "" && h3),
                tabIndex: -1,
              }),
              r2.icon || { class: "no-icon" },
              this.localized ? l1(r2.caption) : Ol(r2.caption)
            );
          })
        ),
        this.other && !this.multiple)
        ...
        (l6.hidden = !h3),
          (l6.value = h3 ? this.value : ""),
          (l6.placeholder = this.placeholder),
          i3.append(l6);
    }
  }
  var vs = N1;
  ...
  class L4 extends u {
    ...
    onResize = () => {
      let { normal: i3, small: l6 } = this.parts,
        s2 = this.offsetParent;
      if (!(s2 instanceof HTMLElement)) return;
      else if (
        s2.offsetWidth < this.minWidth ||
        s2.offsetHeight < this.minHeight
      )
        (i3.hidden = true), (l6.hidden = false), (this.value = "small");
      else (i3.hidden = false), (l6.hidden = true), (this.value = "normal");
    };
    connectedCallback() {
      super.connectedCallback(),
        globalThis.addEventListener("resize", this.onResize);
  ...
  class e0 extends u {
    ...
    static styleSpec = {
      ...
      ":host svg": { width: 16, height: 16, stroke: fM.resizeIconFill },
    };
    content = t3.resize();
    get minSize() {
    ...
    resizeTarget = (i3) => {
      ...
      (l6.style.left = l6.offsetLeft + "px"),
        (l6.style.top = l6.offsetTop + "px"),
        (l6.style.bottom = ""),
        (l6.style.right = "");
      let { minSize: h3 } = this;
      G2(
        ...
        (r2, y2, e2) => {
          if (
            ((l6.style.width = Math.max(h3.width, s2 + r2) + "px"),
            (l6.style.height = Math.max(h3.height, o3 + y2) + "px"),
            e2.type === "mouseup")
          )
            return true;
        },
    ...
    connectedCallback() {
      ...
      let i3 = { passive: true };
      this.addEventListener("mousedown", this.resizeTarget, i3),
        this.addEventListener("touchstart", this.resizeTarget, i3);
    }
  }
  var _s = e0;
  ...
  var P4 = y2.elementCreator({
    ...
    styleSpec: {
      ":host": {
        ...
        "--tag-bg": kE.brandColor("blue"),
        "--tag-text-color": kE.brandTextColor("white"),
        display: "inline-flex",
        borderRadius: kE.tagRoundedRadius(fM.spacing50),
      ...
      ':host [part="remove"]': {
        ...
        borderRadius: kE.tagCloseButtonRadius("99px"),
  ...
  class W1 extends u {
    ...
    addTag = (i3) => {
      let l6 = i3.trim();
      if (l6 === "" || this.tags.includes(l6)) return;
      (this.tags = [...this.tags, l6]), this.queueRender(true);
    };
    toggleTag = (i3) => {
      if (this.tags.includes(i3))
        (this.tags = this.tags.filter((l6) => l6 !== i3)),
    ...
    popSelectMenu = () => {
      let { toggleTag: i3 } = this,
        { tagMenu: l6 } = this.parts,
        s2 = [...this.availableTags],
        o3 = this.tags.filter((r2) => !s2.includes(r2));
      if (o3.length) s2.push(null, ...o3);
      let h3 = s2.map((r2) => {
        if (r2 === "" || r2 === null) return null;
        else if (typeof r2 === "object")
          return {
            checked: () => this.tags.includes(r2.value),
            caption: r2.caption,
            action() {
              i3(r2.value);
            },
    ...
    content = () => [
      x0({ type: "button", style: { visibility: "hidden" }, tabindex: -1 }),
      Zl({
        part: "tagContainer",
        class: "row",
        role: "list",
    ...
    removeTag = (i3) => {
      if (this.editable && !this.disabled) {
        ...
        (this.tags = this.tags.filter((s2) => s2 !== l6.caption)),
          l6.remove(),
          this.queueRender(true);
      }
      i3.stopPropagation(), i3.preventDefault();
    ...
    render() {
      ...
      if (
        ((l6.disabled = this.disabled),
        (s2.value = ""),
        s2.setAttribute("placeholder", this.placeholder),
        this.editable && !this.disabled)
      )
        l6.toggleAttribute("hidden", false),
          s2.toggleAttribute("hidden", !this.textEntry);
      else
  ...
  var Yl = W1.elementCreator({
    ...
    styleSpec: {
      ":host": {
        ...
        gap: fM.spacing25,
        borderRadius: kE.taglistRoundedRadius(fM.spacing50),
        overflow: "hidden",
      },
      ":host[editable]": { gridTemplateColumns: `0px auto ${fM.touchSize}` },
      ...
      ':host [part="tagContainer"]': {
        ...
        borderRadius: kE.tagContainerRadius(fM.spacing50),
  ...
  function El(i3) {
    let { accent: l6, background: s2, text: o3 } = i3,
      h3 = i3.accentText ?? l6.contrasting(),
      r2 = i3.backgroundInset ?? s2.darken(0.03),
      y3 = i3.border ?? o3.opacity(0.15),
      e2 = i3.shadow ?? o3.opacity(0.1),
      x3 = i3.focus ?? l6.opacity(0.5);
    return {
      _tosiAccent: l6,
      _tosiAccentLight: l6.brighten(0.15),
      _tosiAccentDark: l6.darken(0.15),
      _tosiAccentText: h3,
      _tosiBg: s2,
      _tosiBgInset: r2,
      _tosiBgHover: s2.darken(0.05),
      _tosiBgActive: s2.darken(0.1),
      _tosiText: o3,
      _tosiTextMuted: o3.opacity(0.6),
      _tosiTextDisabled: o3.opacity(0.4),
      _tosiBorder: y3,
      _tosiBorderFocus: l6,
      _tosiShadow: e2,
      _tosiShadowColor: e2,
      _tosiFocusRing: `0 0 0 2px ${x3}`,
      _tosiInputBg: s2,
      _tosiInputBorder: y3,
      _tosiInputBorderFocus: l6,
      _tosiButtonBg: s2,
      _tosiButtonText: o3,
      _tosiButtonBorder: y3,
      _tosiButtonHoverBg: s2.darken(0.05),
      _tosiButtonActiveBg: l6,
      _tosiButtonActiveText: h3,
    };
  }
  function t0(i3) {
  ...
  Object.assign(globalThis, {
    tosijs: exports_module,
    tosijsUi: exports_dist,
    tosijsProduct: exports_src,
  });
})();
