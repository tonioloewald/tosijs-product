...
try {
  // 1. Get metadata
  const probe =
    await $`ffprobe -v error -select_streams v:0 -show_entries stream=nb_frames,duration,r_frame_rate -of csv=p=0 ${input}`.text();
  ...
  const output =
    input.replace(/\.[^.]+$/, "") + `_${cols}x${rows}_${total}.webp`;
